...
where type.name = 'Other Income' and line.date <= '""" + self.date_to + """' and move.state = 'posted'"""
        ...
        return self.env.ref('account_custom_report.action_analytic_budget_report').with_context(
            landscape=True).report_action(
            self, data=data)


class analyticBudgetReport(models.AbstractModel):
    _name = 'report.account_custom_report.analytic_budget_report_tamplate'
