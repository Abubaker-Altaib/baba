...
class CrossoveredBudget(models.Model):
    ...
    def onchange_analytic_account_id(self):
        ...
        list_of_tuple =[]
        if self.type == 'out':
            for budget_position in self.analytic_account_id.budget_post_ids :
                if budget_position.type == 'out':
                    list_of_tuple.append((0,0, {
        ...
        self.crossovered_budget_line = list_of_tuple
    ...
    def action_budget_confirm(self):
        ...
        if self.amount != round(sum(self.mapped('crossovered_budget_line.planned_amount' )),2):
            if self.type == 'in':
...
class AccountBudgetLines(models.Model):
    ...
    def create(self, vals):
        """
        overwrite create method to update date_from and date_to from budget dates as default values
        """
        budget = self.env['crossovered.budget'].search([('id','=',vals['crossovered_budget_id'])])
        if not vals['date_from']:
            vals.update({'date_from': budget.date_from})
        if not vals['date_to']:
            vals.update({'date_to': budget.date_to})
      
        res = super(AccountBudgetLines, self).create(vals)
...
class AccountAnalytic(models.Model):
    ...
    analytic_type = fields.Selection([('revenue','Revenue'),('project','Project'),('technical','Technical')], string="Type")
...
class AccountBudgetPost(models.Model):
    ...
    def create(self, vals):
        """
        overwrite create to update account_ids base on account_id
        """
        vals['account_ids'] = [[6, False, [vals['account_id']]]]
        create_id = super(AccountBudgetPost, self).create(vals)
        return create_id

    @api.multi
    def write(self, vals):
        """
        overwrite to update account_ids with the value of account_id
        """
        for rec in self:
            if 'account_id' in vals:
                vals['account_ids'] = [[6, False, [vals['account_id']]]]
            super(AccountBudgetPost, rec).write(vals)
        return True

    @api.model
    def name_search(self, name, args, operator='ilike', limit=100):
        context = self._context
        if 'model' in context and context['model'] == 'account.budget.post':
            if 'field' in context and context['field'] == 'general_budget_id':
                analytic_account_id = self.env['account.analytic.account'].search([('id','=',context['analytic_account_id'])])
                records_ids = [g.id for g in analytic_account_id.budget_post_ids]

                args.append(('id','in',records_ids))

        return super(AccountBudgetPost, self).name_search(name, args, operator, limit)
    ...
    def onchange_account_id(self):
        ...
        if self.account_id:
            self.account_ids = [self.account_id.id]
...
class AccountAccount(models.Model):
    ...
    budget_required = fields.Boolean('Budget Required', default=False)

    @api.multi
    def write(self, vals):
        """"
        check if account is budget required and not analytic account required, 
        then update it with analytic account required
        """
        for account in self:
            analytic_required = 'analytic_required' in vals and vals['analytic_required'] or account.analytic_required or False
            budget_required = 'budget_required' in vals and vals['budget_required'] or account.budget_required or False

            if budget_required and not analytic_required:
                vals['analytic_required'] = True
            
            super(AccountAccount, account).write(vals)
    ...
    def create(self, vals):
        ...
        then update it with analytic account required
        """
        analytic_required = 'analytic_required' in vals and vals['analytic_required'] or False
        budget_required = 'budget_required' in vals and vals['budget_required'] or False
        
        if budget_required and not analytic_required:
                vals['analytic_required'] = True

        create_id = super(AccountAccount, self).create(vals)
        return create_id
    ...
    def invoice_validate(self):
        ...
        for invoice in self:
            ...
            for line in invoice.invoice_line_ids:
                if line.account_analytic_id.budget and line.account_id.budget_required:
