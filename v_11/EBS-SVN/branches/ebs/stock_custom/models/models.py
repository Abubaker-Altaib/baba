    ...
    def button_validate(self):
        ...
        for line in self.move_lines:
            if line.reserved_availability < line.quantity_done:
                raise UserError(_("Done quantity is bigger than Reserved Quantaty!!"))
    ...
    def _compute_rec_name(self):
        for line in self:
            if line.location_id.name:
                line.rec_name = line.location_id.name+'/'+line.name
            else:
                line.rec_name = line.name

    _sql_constraints = [
        ('stock_location_rec_name_uniq', 'unique (rec_name)', _('Parent Location Name & Location Name must be unique.')),]
...
class stockpickingtype(models.Model):
    ...
    def _compute_rec_name(self):
        for line in self:
            if line.warehouse_id.name:
                line.rec_name = line.warehouse_id.name+'/'+line.name
            else:
                line.rec_name = line.name

    _sql_constraints = [
        ('stock_picking_rec_name_uniq', 'unique (rec_name)', _('Warehouse Name & Picking Name must be unique.')),]
...
class stock_scrap(models.Model):
    ...
    def create(self, vals):
        ...
        return super(stock_scrap,self).create(vals)

    def write(self, vals):
        if 'product_id' in vals:
            product = self.env['product.product'].search([('id','=',vals['product_id'])])
            vals.update({'product_uom_id':product.uom_id.id})
        return super(stock_scrap,self).write(vals)
