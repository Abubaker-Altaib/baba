...
class account_asset_operation(models.Model):
    ...
    department_id = fields.Many2one(
        'hr.department', related='asset_id.department_id', string='Department',
        states={'draft': [('readonly', False)]},
        copy=False)
    ...
    def create_move(self):
        ...
        ############### new code 
        #account_asset_id = self.department_id.analytic_account_id.id

        # if self.asset_id.asset_type == 'investment':
        #     account_asset_id = self.category_id.account_investment.id
        # elif self.asset_id.asset_type == 'not_investment':
        #     account_asset_id = self.category_id.account_not_investment.id
    ...
    def done(self):
        for rec in self:
            ...
            elif rec.type in ['change_dept'] :
                rec.asset_id.department_id = self.department_to_id.id
...
class account_asset_depreciation_line(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'Draft'),
         ('validate', 'Validate'),
        ('done','Done')],
        'State', required=True, copy=False, default='draft')

    @api.multi
    def write(self, vals):
        res=super(account_asset_depreciation_line, self).write(vals)

        for rec in self:
            if 'move_check' in vals:
                if rec.move_check == True :
                    rec.state='validate'
            move_id = self.env['account.move'].search([('id','=',rec.move_id.id)])
        ...
        for rec in self :
            if 'state' in vals:
                if vals['state'] == 'posted' :
                    asset = self.env['account.asset.depreciation.line'].search([('move_id','=',rec.id)])
                    if asset :
                        asset.state ='done'
        return res
