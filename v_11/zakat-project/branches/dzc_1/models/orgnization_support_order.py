    ...
    def validate_numbers(self):
        if self.support_method == 'cash':
            ...
            if not re.match("^[0-9]*$", self.estimated_cost.replace(" ", "")):
                raise ValidationError(_('Estimated Cost Cannot be Zero or Less'))
            if self.estimated_cost.replace(" ", "") != self.estimated_cost:
                raise ValidationError(_('Estimated Cost Cannot be Zero or Less'))
            if self.estimated_cost == '0':
                raise exceptions.ValidationError(_("Estimated Cost Cannot be Zero or Less or special Characters"))
