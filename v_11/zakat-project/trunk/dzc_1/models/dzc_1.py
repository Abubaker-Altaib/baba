...
class ZakatApplicationForm(models.Model):
    ...
    create_project = fields.Selection([('n_c','New Collective Project'),('e_c','Existing Collective Project')
    ,('n_i','New Individual Project'),('n_s','New Service Project'),('e_s','Existing Service Project')])
    # 
    ...
    # 
    job = fields.Char(string='Job', related='faqeer_id.job')
    job_type = fields.Selection([('public', 'Public Sector'), ('private', 'Private Sector'),
                                 ...
                                 ('tradesman', 'Tradesman'), ('non', 'Non-Working'), ('others', 'Others/Specify')],
                                'Job Type')
    ...
    # 
    student_ids = fields.One2many('zakat.student.sponsorship', 'fageer_id', string='Students')
    # 
...
class StudentSponsorship(models.Model):
    ...
    @api.constrains('card_id', 'national_number')
    def checks(self):
        """
        ...
        # Regex pattern to check all chars are integers
        pattern = re.compile(r'^[0]\d{9,9}$')
        if self.card_id != False:
            if not pattern.search(self.card_id):
                raise exceptions.ValidationError(_('card_id 1 must be exactly 10 Numbers and Start with ZERO 0 .'))
        if self.national_number != False:
            if 11 < len(self.national_number) or 11 > len(self.national_number):
                raise exceptions.ValidationError(_('ID Number must be at least 11 Numbers.'))

    @api.constrains('extract_date')
    def check_date(self):
        """
        To check date fields
        :return: raise exceptions
        """
        extract_date = datetime.strptime(self.extract_date, '%Y-%m-%d')
        now = datetime.now()
        # if self.date_of_arrival < self.date:
        #     raise exceptions.ValidationError(_("Date Of Arrivals Must Be With In The Current Date"))
        if extract_date.year >= now.year:
            raise exceptions.ValidationError(_('Birth Date Must Be Less Than Current Date'))
...
class EmergencyandUrgentCases(models.Model):
    ...
    decision = fields.Selection([('agree', 'Agree'), ('apology', 'Apology')])
