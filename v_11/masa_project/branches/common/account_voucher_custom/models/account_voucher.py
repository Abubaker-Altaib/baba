    ...
    def pay(self):
        for payment in self:
            if payment.voucher_ids:
                payment.voucher_ids.write({'state':'paid'})
...
class AccountVoucher(models.Model):
    ...
    payment_journal_id = fields.Many2one('account.journal', compute=False,string='Payment Method', readonly=True,store=True,
    ...
    @api.multi    
    def budget_confirmation_create(self):
        for voucher in self:
            if voucher.journal_id.type == 'purchase':
                for line in voucher.line_ids:
                    if not line.account_analytic_id or not line.account_id.budget_account:
                        continue
                    ...
                    vals = {
                         ...
                         'account_id': line.account_id.id,
                         'date': voucher.account_date or voucher.date,
                    ...
                    if line.budget_confirm_id:
                        line.budget_confirm_id.write(vals)
                        new_confirm_id = line.budget_confirm_id
                    else:
                        new_confirm_id = self.env['account.budget.confirmation'].create(vals)
                        line.write({'budget_confirm_id':new_confirm_id.id})
                    if voucher.company_id.auto_budget:
    ...
    def action_check_budget(self):
        ...
        if self.check_lines or not line.account_analytic_id or not line.account_id.budget_account:
    ...
    def proforma_voucher(self):
        if any(voucher.state != 'paid' for voucher in self):
    ...
    def return_amount_to_text(self,amount,currency):
        ...
        @return: string
        """
        return amount_to_text_ar.amount_to_text(amount, 'ar',currency.currency_unit_label,currency.currency_unit_label)
...
class AccountVoucherLine(models.Model):
    ...
    budget_confirm_id= fields.Many2one('account.budget.confirmation', 'Confirmation', copy=False, ondelete="restrict")
    state=fields.Selection([
    ...
    def onchange_account_id(self):
        if self.account_id and self.account_id.analytic and self.account_id.analytic_account_ids:
            ...
            return {'domain': {'account_analytic_id': [('id', 'in', analytics)]}}
...
class AccountBudgetOperation(models.Model):
    ...
    voucher_id = fields.Many2one('account.voucher', string='voucher',readonly=True,states={'draft':[('readonly', False)]})
