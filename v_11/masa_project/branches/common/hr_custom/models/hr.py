...
class Employee(models.Model):
    ...
    qualification_id = fields.Many2one('hr.employee.qualification',string='Highest qualification',compute='_get_high_qual')
    ...
    sinid_date = fields.Date('SIN Date', help='Social Insurance Date', groups="hr.group_hr_user")

    @api.multi
    @api.depends('qualification_ids')
    def _get_high_qual(self):
        for emp in self:
            emp.qualification_id=self.env['hr.employee.qualification'].search([('id', 'in', emp.qualification_ids.ids)],limit=1,order='degree_id').id
