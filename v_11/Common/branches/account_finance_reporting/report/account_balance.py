...
class ReportTrialBalance(models.AbstractModel):
    ...
    def _get_accounts(self, accounts, display_account, level):
        ...
        init_tables, init_where_clause, init_where_params = MoveLine.with_context(date_from=self.env.context.get('date_from'), date_to=False)._query_get()
        init_tables = init_tables.replace('"','')
        init_where_clause = '("account_move_line"."move_id"="account_move_line__move_id"."id") AND ((("account_move_line"."date" < %s)  AND  ("account_move_line"."journal_id" in (%s,%s,%s,%s,%s,%s,%s)))  AND  ("account_move_line__move_id"."state" = %s))'
        ...
        print ('acc_by_level----------->', acc_by_level)
        for account in acc_by_level:
            ...
            print ('--------------------------')
            print ('init_account_result', init_account_result)
