...
class sale_category(osv.osv):
	...
	for record in self.browse(cr, uid, ids):
		...
		for line in record.product_ids:
			...
			cr.execute(""" update product_template set cost_method ='standard', list_price=%s, standard_price=%s
 where id=%s""",(line.total_amount,line.total_amount,line.product_id.product_tmpl_id.id))
...
class sale_order_line(osv.Model):
	...
	if record.order_id.payment_type != 'cash':
		...
		cr.execute(""" update sale_order_line set period = %s , price_unit=%s, installment_value =%s, up_front =%s ,total_price=%s where id=%s""",(record.period , line.total_amount , line.total_amount , record.up_front , line.total_amount,record.id))
    ...
    _columns = {
        ...
        'product_uom_qty': fields.float('Quantity',required=True, digits=(16,0),readonly=True, states={'draft': [('readonly', False)]}),
    ...
    """def button_confirm(self, cr, uid, ids, context=None):
	    ...
	    if line.price_unit<0 or line.product_uom_qty<0 or line.product_uos_qty<0 or line.discount<0:
		raise osv.except_osv(_('Error!'),_('You can not insert negative amount in the order line.'))
  
        return super(sale_order_line, self).button_confirm(cr, uid, ids, context)"""

    """def onchange_period(self, cr, uid, ids, period):
	result = {}
	val = 0.0
        if period:
		for line in self.browse(cr, uid,ids):
            		if period > line.period : 
				raise osv.except_osv(_('Error!'),_('You can not increase installment.'))
            			result['period']= line.period
				result['price_unit'] = line.total_price
				result['installment_value']=line.total_price
			else : 
            			val = (line.total_price - line.up_front ) / period
            			result['period']= period
				result['price_unit'] = val
				result['installment_value']=val
        return {'value': result}

    #def onchange_up_front(self, cr, uid, ids, up_front):
	result = []
	val = 0.0
        if up_front:
		for line in self.browse(cr, uid,ids):
            		if up_front < line.up_front :
				raise osv.except_osv(_('Error!'),_('You can not Decrease up front.'))
            			result['up_front']= line.up_front
				result['price_unit'] = line.total_price
				result['installment_value']=line.total_price
			else : 
            			val = (line.total_price - up_front ) / line.period
            			result['up_front']= up_front
				result['price_unit'] = val
				result['installment_value']=val
        return {'value': result}"""
	...
	for line in catgory_obj.product_ids :
		if line.product_id.id == product :
			if flag == 'cash':
        		...
        		result['period'] = line.installment
...
class sale_order(osv.Model):
    ...
    _inherit = "sale.order"
    _columns = {
        'employee_id' : fields.many2one('hr.employee','Customer name',select=True, invisible=False, required=True,readonly=True,states={'draft': [('readonly', False)]},track_visibility='onchange',),
       ...
       'product_qty' : fields.integer('Product QTY',track_visibility='onchange',readonly=True , states={'draft': [('readonly', False)]}),
		...
		for record in self.browse(cr, uid, ids):
			...
			if line.order_id.prepare == False :
				""" 1- To check Qty on hand and future qty (not done , cancel) picking
			    		2- check product limit  
				""" 
	  			check_move_ids = stock_move_obj.search(cr,uid,[('product_id','=',line.product_id.id),('state','not in',('done','cancel')),('picking_id','!=',False),('location_id','=',record.shop_id.warehouse_id.lot_stock_id.id)],context=context)
		    ...
		    current_salary = payroll_obj.current_salary_status(cr, uid,ids, record.employee_id, record.date_order)
    ...
    def stock_loan_create(self, cr, uid, ids, picking_created=False, context=None):
        payroll_obj = self.pool.get('payroll')
	...
	for order in self.browse(cr, uid, ids, context):
		...
		if order.payment_type == 'installment' :
                	current_salary = payroll_obj.current_salary_status(cr, uid,ids, order.employee_id, order.date_order)
                	balance = current_salary.get('balance',0.0)
		...
		#self._create_pickings_and_procurements(cr, uid, order, order.order_line, None, context=context)
                	u = self.pool.get('res.users').browse(self, cr, uid, ids).id
        		self.write(cr, uid, ids, {'state':'done','approve_id':u,'print_order':False}, context=context)
        	else :
                	u = self.pool.get('res.users').browse(self, cr, uid, ids).id
			self.write(cr, uid, ids, {'state':'invoice','approve_id':u,'print_order':False}, context=context)
        return True
	...
	date = time.strftime('%Y-%m-%d')
        for record in self.browse(cr, uid, ids):
            current_salary = payroll_obj.current_salary_status(cr, uid,ids, record.emp_code, date)
            balance = current_salary.get('balance',0.0)
	    if balance<record.amount_total:
		raise osv.except_osv(_('Error!'),_('this employe is not enough balance .'))
    ...
    """def button_done(self, cr, uid, ids, context=None):
        ...
        return sale_obj.manual_invoice(cr, uid, ids, context)"""
	...
	for order in self.browse(cr, uid, ids, context):
			...
			if line.order_id.prepare == False :
            			location_id = line.order_id.shop_id.warehouse_id and line.order_id.shop_id.warehouse_id.lot_stock_id.id
            			if location_id and self.check_access_rule_location(cr, uid, [location_id], 'read')==True:
					res = stock_location_obj._product_get(cr, uid, location_id, [line.product_id.id], context = context)[line.product_id.id]
				if res <= 0 :
					raise osv.except_osv(_('Error'), _('Your Product Qty is out of stock\n %s')% (line.product_id.name))
				if res < line.product_uos_qty :
					raise osv.except_osv(_('Error'), _('Your Product Qty is not right please check it again\n %s')% (line.product_id.name))
				if line.product_uos_qty <= 0 :
					raise osv.except_osv(_('Error'), _('Your Product Qty is out of stock\n %s')% (line.product_id.name))

				if res <= line.min_roof:
					raise osv.except_osv(_('Error'), _('Your Product Qty is reach the limit contact adminstrator\n %s')% (line.product_id.name))
                current_salary = payroll_obj.current_salary_status(cr, uid,ids, order.employee_id, order.date_order)
                balance = current_salary.get('balance',0.0)
	        if order.employee_id.salary_suspend == True:
		...
		if order.payment_type =='installment':
				...
				for order_line in order.order_line:
					up_front += order_line.up_front
				if up_front > 0.0 :
                			inv_id = inv_obj.create(cr, uid, invoice_dict, context=context)
					for order_line in order.order_line:
						amount = order_line.up_front * order_line.product_uom_qty
            	     				inv_line_values = inv_line_obj.create(cr, uid, {'name': order.name,
                					'origin': order.name,
                					'account_id': order.category_id.account_id.id,
                					'price_unit': amount,
							'invoice_id':inv_id,
                					'quantity': order_line.product_uom_qty or 1.0,
                					'discount': False,
                					'uos_id': order_line.product_uos.id,
                					'product_id': order_line.product_id.id,
                					#'invoice_line_tax_id': res.get('invoice_line_tax_id'),
                					'account_analytic_id': order.shop_id.project_id.id or False,
            		    				}, context)
						inv_ids.append(inv_id)
             		else:
             			raise osv.except_osv(_('Error!'),_('No loan created, Please creat a loan in the company form in Hr setting page .'))
		elif order.payment_type =='cash':
			inv_id = inv_obj.create(cr,uid ,{'name': order.name,\
                		'origin': order.name,\
                		'type': 'out_invoice',\
                		'reference': False,\
                		'account_id': order.partner_id.property_account_payable.id,
		...
		#self._create_pickings_and_procurements(cr, uid, order, order.order_line, None, context=context)
                u = self.pool.get('res.users').browse(self, cr, uid, ids).id
        	self.write(cr, uid, ids, {'state':'done','approve_id':u,'print_order':False,'invoice_ids':[(6, 0,inv_ids)]}, context=context)
        return True"""
