...
# Method to get data      
      def get_sale_order_data(self,data):
           ...
           if category_id :
              		conditions = conditions + " and so.product_cat=(%s)"%category_id[0] 
           if location_id :
		...
		if  payment_type == 'installment' : 
           		self.cr.execute( """
					...
					left join sale_order_line l on (so.id = l.order_id)
                            where (to_char(so.create_date,'YYYY-mm-dd')>=%s and to_char(so.create_date,'YYYY-mm-dd')<=%s) and so.payment_type in ('installment','up_cash') and so.state in ('done') 
           		...
           		self.cr.execute( """
					...
					so.state as state ,
                                ...
                                From sale_order so
                                	...
                                	left join product_category cat on (so.product_cat=cat.id)
					left join sale_order_line l on (so.id = l.order_id)
                            where (to_char(so.create_date,'YYYY-mm-dd')>=%s and to_char(so.create_date,'YYYY-mm-dd')<=%s) and so.state in ('done') 
