...
class account_employee_custody(report_sxw.rml_parse, common_header, custom_common_header):
    ...
    def _get_partner(self, data):
        ...
        partner_ids = [-1]
        if line_ids and data['form']['group_report']:
            ...
            sql_query = """SELECT  p.id as id, coalesce(d.id,0) as d_id  \
                             FROM   res_partner p
                             LEFT JOIN hr_salary_degree d ON  p.degree_id = d.id \
    ...
    def lines(self, data, partner_id):
        ...
        sql_query = """SELECT l.amount as amount, v.chk_seq AS chk_seq,v.number as ref,l.permission as permission, \
...
report_sxw.report_sxw('report.employee.custody', 'account.account', 'addons/account_report_niss/report/account_custody_report.rml', parser= account_employee_custody, header=False)
