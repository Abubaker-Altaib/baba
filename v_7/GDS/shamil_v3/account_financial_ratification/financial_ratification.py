...
class account_voucher(osv.Model):
    ...
    def write(self, cr, uid, ids,vals, context=None):
        ...
        if voucher:
            if currency_format=='ar':
               currency=self.pool.get('res.currency').read(cr, uid, voucher.currency_id.id, ['units_name','cents_name'], context=context)
               amount_in_word=amount_to_text_ar(voucher.amount, currency_format, currency['units_name'], currency['cents_name'])
            else:
    ...
    def pre_post(self, cr, uid, ids, context={}):
        ...
        return True
    def test_wkf(self, cr, uid, ids, context=None):
        for voucher in self.browse(cr, uid, ids):
            if  voucher.purpose.special:
                 return True
            return False
    	...
    	for voucher in self.browse(cr, uid, ids, context=context):
            ...
            if voucher.purpose:
                ...
                voucher_line = {
                    ...
                    'account_id': voucher.purpose.account_id.id or False,
        	    }


    	        if voucher.line_ids :
                   for line in voucher.line_ids:
        		       self.pool.get('account.voucher.line').write(cr, uid, line.id, voucher_line, context=context)
    	        else:

        		   new_voucher_line = self.pool.get('account.voucher.line').create(cr, uid, voucher_line, context=context)

    	self.write(cr, uid, ids, {'state': 'preapprove','type':'purchase','ratification':True})
        cxt = context.copy()
        cxt.update({'type':'ratification'})
        if not super(account_voucher, self).create_budget_confirmation(cr, uid, ids, context=cxt):
    ...
    def onchange_partner_id_ratification(self, cr, uid, ids, partner_id, journal_id, ttype, price, context={}):
        ...
        default['value']['journal_id'] = journal_id and journal_id or self._get_journal(cr, uid, context=context)
    ...
    def _required_line_id(self, cr, uid, ids, context=None):
        ...
        """
        if self.search(cr, uid,[('id', 'in', ids),('line_ids', '=', False),('state', '=', 'posted')], context=context):
            return False
        return True
