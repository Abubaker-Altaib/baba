...
class exchange_order(osv.Model):
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            res[order.id] = { 'total_amountl': 0.0}
            val = 0.0
    ...
    _columns = {
        ...
        'ttype': fields.selection([('other', 'Other'), ('store', 'Stock Store'), ('return','Return')], 'Type', required=True, select=True),
        'location_id': fields.many2one('stock.location', 'Location', readonly=True, states={'draft': [('readonly', False)]}),
        'location_dest_id': fields.many2one('stock.location', 'Destination Location', readonly=False, domain="[('usage','in',['internal'])]",),
        ...
        'total_amount': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Exchange Price'), string='Amount',
            store={
                'exchange.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line'], 10),
                'exchange.order.line': (_get_order, ['price_unit', 'approved_qty'], 11),
            },multi='sums'),
    ...
    def action_approve_order(self, cr, uid, ids, context=None):
        ...
        """
        for order in self.browse(cr, uid, ids, context=context):
            if order.state == 'category_manager' and order.ttype == "store" and order.categ_id:
               if uid not in [manager.id for manager in order.categ_id.manager_ids]: 
                   raise osv.except_osv(_('Error !'), _('You can not approve this order because you are not responsible for this product category'))
    ...
    def has_category_manager(self, cr, uid, ids, context=None):
        ...
        """
        for order in self.browse(cr, uid, ids, context=context):
            if order.ttype == "store" and order.categ_id and order.categ_id.manager_ids:
                return True
        return False
    ...
    def action_cancel_draft(self, cr, uid, ids, *args):
        ...
        for p_id in ids:
            ...
            for line in self.browse(cr, uid, p_id).order_line:
                order_line_obj.write(cr, uid, line.id, {'delivered_qty':0.0 })
    ...
    def action_process(self, cr, uid, ids, context=None):
        ...
        picking_id = None
        for order in self.browse(cr, uid, ids, context):
            if order.ttype == 'return':
                pick_name = self.pool.get('ir.sequence').get(cr, uid, 'stock.picking.in')
                prepare_order_picking = self._prepare_order_picking(cr, uid, order, context=context)
                # Change picking type to in, order type is return
                prepare_order_picking['type'] = 'in'
                prepare_order_picking['name'] = pick_name
                prepare_order_picking['partner_id'] = order.partner_id.id
                todo_moves = []
                for line in order.order_line:
                    product_qty = line.approved_qty
                    # If there is no quantity in the line just skip it
                    if product_qty == 0:
                        continue
                    if not picking_id:
                        picking_id = picking_obj.create(cr, uid, prepare_order_picking, context=context)
                    prepare_stock_move = self._prepare_order_line_move(cr, uid, order, line, picking_id,product_qty ,context=context)
                    # Swap location_id and location_dest_id because picking type is in
                    temp_location_id = prepare_stock_move['location_id']
                    prepare_stock_move['location_id'] = prepare_stock_move.get('location_dest_id')
                    prepare_stock_move['location_dest_id'] = temp_location_id
                    move_id = stock_move_obj.create(cr, uid, prepare_stock_move, context=context)
                    todo_moves.append(move_id)
                    line_obj.write(cr, uid, [line.id], {'state' :'picking','delivered_qty': product_qty})
                stock_move_obj.action_confirm(cr, uid, todo_moves)
                stock_move_obj.force_assign(cr, uid, todo_moves)
                if picking_id:
                    wf_service.trg_validate(uid, 'stock.picking', picking_id, 'button_confirm', cr)
                else:
                    raise osv.except_osv(_('Error!'), _('At least one line must have approved quantity bigger than equal.'))   
   
                self.write(cr, uid, ids, {})
                return res
        # If type order is not return the code will continue here
    ...
    def _prepare_order_line_move(self, cr, uid, order, line, picking_id,product_qty ,context=None):
        ...
        """
        if order.ttype =="other" and not order.stock_journal_id.location_id:
            raise osv.except_osv(_('Error!'), _('Please add location in stock journal "purpose".'))      
        return {
            'name': line.name[:250],
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                ...
                if not picking_id and product_qty != 0:
                    ...
                    prepare_order_picking = self._prepare_order_picking(cr, uid, order, context=context)
                    prepare_order_picking['name'] = pick_name
                    picking_id = pick_obj.create(cr, uid, prepare_order_picking,context=context)
                if product_qty != 0:
...
class exchange_order_line(osv.Model):
    ...
    def _amount_line(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            price = line.product_id.standard_price * line.approved_qty or 0.0
            res[line.id] = price
        return res
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        ...
        and it inherit to change view dynamically
        ...
        if context is None:
            context = {}
        if view_type=='form': 
            res = super(exchange_order_line,self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
            view_id = self.pool.get('ir.ui.view').search(cr,uid,[('name', '=', 'exchange.order.line.form')]) or None
            if res['view_id']==view_id:
                view_id = self.pool.get('ir.ui.view').search(cr,uid,[('name', '=', 'exchange.order.line.form')])
                rec_by_categ=self.pool.get('res.users').browse(cr, uid, uid).company_id.rec_by_categ
                if rec_by_categ == True :
                    view_id = self.pool.get('ir.ui.view').search(cr,uid,[('name', '=', 'exchange.order.line.form.domain')])
            view_id = view_id and view_id[0] or None  
        return super(exchange_order_line,self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
         
    _columns = {
        'name': fields.char('Description', size=450, required=True, readonly=True, states={'draft': [('readonly', False)]}),
    ...
    def onchange_product_qty(self, cr, uid, ids, product_qty):
        """
        Changing allow_req field value
        @paapproved_qtyram product_qty:product_qty
        @return :dictionary conatins the new values of:
