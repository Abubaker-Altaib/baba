...
class account_voucher(osv.Model):
        ...
        for voucher in self.pool.get('account.voucher').browse(cr, uid, ids, context=context):
            for line in voucher.line_ids:
                if line.budget_confirm_id:
                   self.pool.get('account.budget.confirmation').write(cr, uid, line.budget_confirm_id.id,{'state': 'cancel'}, context=context)
    ...
    def cancel_voucher(self, cr, uid, ids, context={}):
        ...
        self.write(cr, uid, ids, {'state': 'cancel', 'move_id': False, 'active':1}, context=context)
        return True
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            # Create Move Line for each voucher line'''
                ...
                ml_id = move_line_pool.create(cr, uid,{
                    'name': voucher.name or voucher.narration or '/',
                    'debit': debit,
                    'credit': credit,
                    'account_id': account_id ,
                    'move_id': move_id,
            ...
            for line in voucher.line_ids:
                ...
                move_line = {
                    'journal_id': journal_id.id,
                    'period_id': period_id.id,
                    'name': voucher.narration or line.name and line.name or '/',
                    'account_id': line.account_id.id,
                    'move_id': move_id,
                    'partner_id': voucher.partner_id.id,
                    'currency_id': company_currency != current_currency and current_currency or False,
