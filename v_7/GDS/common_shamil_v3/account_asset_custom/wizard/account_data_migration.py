...
class account_post_move(osv.osv_memory):
    ...
    def trans_rec_reconcile_full(self, cr, uid, ids, context=None):
        ...
        if 'active_ids' in context and context['active_ids']:
            ...
            for rec in data:        
                cat_id = category_obj.search(cr, uid, [('code', '=', rec.categ_code), ('company_id','=',wiz_obj.journal_id.company_id.id) ])  
                loc_id = location_obj.search(cr, uid, [('code', '=', rec.location_code) ]) 
                if not loc_id:
                ...
                history_opration_ids.append(history_id)
                if rec.revalue_amount >  0:
                    ...
                    history_opration_ids.append(history_id)
                asset_obj.validate(cr,uid,[asset_id])
            ...
            if asset_ids:
                history_obj.create_operation_move(cr,uid,history_opration_ids)   
                depreciation_line_obj.create_move( cr, uid, dprc_line_ids, context={})
                asset_obj.compute_depreciation_board(cr,uid,asset_ids)
