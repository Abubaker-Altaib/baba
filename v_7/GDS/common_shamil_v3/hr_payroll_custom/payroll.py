...
class payroll(osv.osv):
    ...
    def family_relation_calculation(self, cr, uid, emp_id, date):
       ...
       rel_ids = family_relation_obj.search(cr, uid, [('employee_id', '=', emp_id), ('start_date', '<=', date), ('state', '=', 'approved')])
       rel_amount = 0.0
       wife_amount = 0.0
       ch_no = 0
       if rel_ids:
          for rel in family_relation_obj.browse(cr, uid, rel_ids):
             if not rel.end_date or rel.end_date > date :
                if rel.relation_id.max_age > 0:
                   ...
                   if rel.birth_date:
                      ...
                      if age <= rel.relation_id.max_age:
                         rel_amount += rel.relation_id.amount
                         ch_no += 1
                else:
    ...
    def allowances_deductions_sp_calculation(self, cr, uid, emp_dict, allow_list):
       ...
       tax_amount = 0.0
       allow_deduct_sp_ids = allow_deduct_exception_obj.search(cr, uid, ['|', ('end_date', '>', emp_dict['date']), ('end_date', '=', False), ('allow_deduct_id', '=', emp_dict['allow_deduct'].allow_deduct_id.id), ('employee_id', '=', emp_dict['emp_id']), ('start_date', '<=', emp_dict['date']), ('action', '=', 'special')])
    ...
    def write_allow_deduct(self, cr, uid, emp_id, result_dict , emp_obj=False):
       ...
       for res in result_dict:
          ...
          check_allow = employee_salary_obj.search(cr, uid, [('employee_id', '=', emp_id), ('allow_deduct_id', '=', res['allow_deduct'].allow_deduct_id.id)])
          ...
          if check_allow:
             if res['amount'] > 0:
                update = employee_salary_obj.write(cr, uid, check_allow, allow_deduct_dict)
             else:
                delete = employee_salary_obj.unlink(cr, uid, check_allow)
          else:
             if res['amount'] > 0:
                create = employee_salary_obj.create(cr, uid, allow_deduct_dict)
       delete = employee_salary_obj.unlink(cr, uid, unlink_ids)
       return True
   ...
   # def change_allow_deduct(self, cr, uid, allow_deduct_id, scale_allow_deduct_id,emp_obj=False):
    '''def change_allow_deduct(self, cr, uid, allow_deduct_id, scale_allow_deduct_id ,emp_obj=False):
        ...
        emp_ids = []
        if scale_allow_deduct_id:
            for rec in salary_allow_deduct_obj.browse(cr, uid, scale_allow_deduct_id):
                if rec.allow_deduct_id.id not in new_allow_deduct_ids:
                    new_allow_deduct_ids.append(rec.allow_deduct_id.id)
        if allow_deduct_id:
            for x in allow_deduct_id:
        ...
        for allow_deduct_rec in allow_deduct_obj.browse(cr,uid,new_allow_deduct_ids):
            ...
            if allow_deduct_rec.in_salary_sheet:
                ...
                scale_allow_deduct_id = salary_allow_deduct_obj.search(cr, uid, [('allow_deduct_id', '=', allow_deduct_rec.id)])    
                if scale_allow_deduct_id:
                    for allow_deduct in salary_allow_deduct_obj.browse(cr, uid, scale_allow_deduct_id):
                ...
                if emp_ids:
                    for emp in employee_obj.browse(cr, uid, emp_ids): 
                        allow_deduct_dict = self.allowances_deductions_calculation(cr, uid, date, emp, {}, [allow_deduct_rec.id], False, [])
