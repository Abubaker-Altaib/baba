...
class hr_employee(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        """Method that overwrites write method and checks if any changes happend in the fields that related to salary then calls 
           ...
           @return: Boolean True
        """
        #raise osv.except_osv(_('Warning!'), _('You cannot %s') %vals)
        ...
        user_update_field = [key for key in vals.keys() if key in ('emp_code', 'name','company_id')]
        if update_field:
            update = self.write_employee_salary(cr, uid, ids, [])
        if user_update_field:
           for emp in self.browse(cr, uid, ids, context=context):
               emp_code = vals.has_key('emp_code') and vals['emp_code'] or emp.emp_code
               name_related = vals.has_key('name') and vals['name'] or emp.name
               company_id = vals.has_key('company_id') and vals['company_id'] or emp.company_id.id
               user_id = emp.user_id and emp.user_id or False
               #partner_id = user_id.partner_id and user_id.partner_id or False
               if user_id:
                  user_obj.write(cr,uid,[user_id.id],{'login':emp_code,'name':name_related,'company_id':company_id})
               #if partner_id:
               #   partner_obj.write(cr,uid,[partner_id.id],{'login':emp_code,'name':name_related,'company_id':company_id})
        return emp_write
...
class hr_employee_reemployment(osv.Model):
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids):
            if record.reemployment_date < record.employee_id.end_date:
		        raise orm.except_orm(_('Warning!'), _('The start date must be after re-employee end date'))
