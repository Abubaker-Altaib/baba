...
class hr_allowance_deduction_report(osv.osv_memory):
    ...
    _columns = {
        ...
        'department_ids' : fields.many2many('hr.department' , 'hr_report_custom_deps_pay_rel', 'report_id','dep_id', string="groups") ,
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if not data['allow_deduct_ids']:
            ...
            if not data['in_salary_sheet']:
                domain += [('allowance_type','=','general'),('allowance_type','!=','in_cycle'),('special','=', True)]
            if data['type']:
        ...
        company_name = self.pool.get('res.company').read(cr , uid , data['company_id'] , ['name'])[0]['name']
