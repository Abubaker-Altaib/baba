    ...
    def test_state(self, cr, uid, ids, mode, context=None):
        ...
        for line in voucher.line_ids:
            if (not line.budget_confirm_id) or (line.budget_confirm_id.state in ['valid','cancel']):
                ...
                wf_service.trg_validate(uid, 'account.budget.confirmation', line.budget_confirm_id.id, 'complete', cr)
                wf_service.trg_validate(uid, 'account.budget.confirmation', line.budget_confirm_id.id, 'check', cr)
                #raise osv.except_osv(_('Error!'), _('there is no Budget for this account!'))
                #finished = False
            if line.budget_confirm_id:
