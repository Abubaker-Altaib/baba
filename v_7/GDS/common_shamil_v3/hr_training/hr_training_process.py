...
class hr_employee_training(osv.Model):
    ...
    _columns = {
        ...
        'partner_id': fields.many2one('res.partner', 'Trainer', ondelete='restrict',
                                     states={'draft':[('readonly', False)]}),
    ...
    def check_account(self, cr, uid, rec, context=None):
		...
		@return: Boolean True or False
        """
        if not rec.company_id.training_account_id or not rec.company_id.training_analytic_account_id or \
...
class hr_employee_training_line(osv.Model):
    ...
    _columns = {
        ...
        'enrich_payment_no': fields.related('training_employee_id', 'enrich_payment_no', type='char', string='Enrich Payment Number', readonly=True),
        ...
        'plan_id': fields.related('training_employee_id', 'plan_id', type='char', string='Plan', store=True, readonly=True),
    ...
    def _check_holidays(self, cr, uid, ids, context=None):
        ...
        line = self.browse(cr, uid, ids[0], context=context)
        if line.training_employee_id.state in ['approved','done']:
    ...
    _constraints = [
        (_check_days, _('The attending number of days for each employee shouldn\'t be grater than the total course number of days'), []),
        (_check_holidays, _('This employee in holiday'), ['employee_id']),
...
class hr_employee(osv.Model):
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if 'department_ids' in context:
            ...
            args.append(('department_id', 'in', [isinstance(d['department_id'], tuple) and d['department_id'][0] or d['department_id'] for d in department_ids]))
