...
class stock_location_product(osv.osv_memory):
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        product_ids = self.pool.get('product.product').search(cr, uid, [('type', '=', 'product')], context={'active_test': False})
        if data['category_id']:
            product_ids = self.pool.get('product.product').search(cr, uid, [('type', '=', 'product'),('categ_id', 'child_of', data['category_id'][0])], context={'active_test': False})

        # GET DEPARTMENT CHILDS IF ANY 
        department_ids=[]
        ...
        return {
            ...
            'report_name': 'stock.location.product2',
            'datas': datas,
            }
