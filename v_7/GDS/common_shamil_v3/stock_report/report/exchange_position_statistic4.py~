...
class exchange_position_statistic4(report_sxw.rml_parse):
      def __init__(self, cr, uid, name, context):
        super(exchange_position_statistic4, self).__init__(cr, uid, name, context)
      ...
      def get_move_data(self,data,department_id):
           ...
           if department_id:
              conditions = conditions + " and po.department_id=(%s)"%department_id
           self.cr.execute( """
                select            distinct po.department_id as dep_id,    
                            ...
                            where (to_char(pick.date,'YYYY-mm-dd')>=%s and to_char(pick.date,'YYYY-mm-dd')<=%s)   and pick.state = ('done') 
                           """ + conditions + """ group by po.department_id,pdc.id,p_temp.name ,p_temp.categ_id order by qty   """  ,(date1,date2)) 
           res = self.cr.dictfetchall()
	   if cat_id :
	      cat_ids =  type_obj.search(self.cr, self.uid, [('id', 'child_of',cat_id[0])])		
              res=[x for x in res if x['cat_id'] in cat_ids]
...
report_sxw.report_sxw('report.exchange_position_statistic_report4', 'stock.picking', 'addons/stock_report/report/exchange_position_statistic4.rml' ,parser=exchange_position_statistic4,header='internal landscape')
