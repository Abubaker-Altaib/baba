...
class payroll_listing_salary(report_sxw.rml_parse):
    ...
    def payroll(self,data,lis,payroll):
        self.cr.execute(
            ...
            'pay.net AS net , d.sequence AS degree_seq  '\
           ...
           # 'AND pay.in_salary_sheet = True '\
            ...
            'AND sal.id = %s'\
            'AND pay.arch_id = %s'\
            'ORDER BY  d.sequence desc' , (data['form']['month'],data['form']['year'],lis,payroll , data['ids']))    
        ...
        for r in res:
          ...
          self.total['net'] += int(r['net'])
        temp = sorted(res, key=lambda k: long(k['code'])) #sorting using code
