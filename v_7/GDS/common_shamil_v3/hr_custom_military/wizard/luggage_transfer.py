...
class luggage_transfer(osv.osv_memory):
...
class emp_luggage_transfer_hours(osv.osv_memory):
    ...
    _columns = {
        ...
        'emp_code': fields.char('Code', size=64),
    ...
    def onchange_name_code(self, cr, uid, ids, emp, code=True):
        ...
        if code:
            emp_id = employee_tab.search(cr, uid, [('emp_code', '=', emp)])
            if emp_id:
            ...
            result['value'] = {'emp_code': employee_tab.browse(cr, uid, [emp])[
                0].emp_code, }
        return result
