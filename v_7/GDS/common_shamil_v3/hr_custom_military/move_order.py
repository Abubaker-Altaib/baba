...
class Move_order_line(osv.Model):
    ...
    _columns = {
        ...
        'type' : fields.selection(move_order_types , string='Type'),
        'date' : fields.date('Date') ,
    }

    def name_get(self, cr, uid, ids, context=None):
        return ids and [(item.id, "%s-%s-%s" % (item.move_order_id.source.name , item.move_order_id.destination.name
 , item.move_order_id.date)) for item in self.browse(cr, uid, ids, context=context)] or []
    def onchange_line(self , cr , uid , ids ,order_type ,employee_id,move_date , context=None):
        ...
        try:
            ...
            print "###################### exception in fucntion onchange_type in 'hr.move.order.line' "
            traceback.print_exc()
...
class hr_move_order(osv.Model):
    ...
    _columns = {
        ...
        'type' : fields.selection(move_order_types , string="Type", states={'confirm': [('readonly', True)]}),
        ...
        for rec in self.browse(cr , uid , ids):
            if rec.type :
                for line in rec.move_order_line_ids :
                    if rec.type in move_order_types_dict:
		            type_obj = move_order_types_dict[rec.type]
		            if line[type_obj[1]]:
		                self.link_2_model(cr , uid , type_obj[0] , type_obj[2],line[type_obj[1]].id , line.id)
        return self.write(cr , uid, ids , {'state' : 'confirm'})

class hr_move_order_advance(osv.Model):
    _name = 'hr.move.order.advance'
    _inherit = 'hr.move.order'
    _columns = {
        'company_id' : fields.many2one('res.company'  , string='Unit Name') ,
        'power' : fields.char('Power Name') ,
        ...
        'type' : fields.selection([('opreation' , 'Opreation') , ('mission' , 'Mission')], string="Reason") , 
    }
     

    def name_get(self, cr, uid, ids, context=None):
        return ids and [(item.id, "%s-%s-%s" % (item.power , item.destination , item.date)) for item in self.browse(cr, uid, ids, context=context)] or []
