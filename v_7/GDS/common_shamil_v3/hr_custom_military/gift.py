...
class hr_medal(osv.Model):
	...
	_sql_constraints = [
	    ('model_uniq1', 'unique(code)', _('code is already exist')),
        ('model_uniq', 'unique(name)', _('name is already exist')),
 	]

	def unlink(self, cr, uid, ids, context=None):
		for rec in self.browse(cr, uid, ids):
			if rec.employee_medals_ids:
				raise osv.except_osv( _(''), _("can not delete record linked with other record"))
...
class hr_employee_medal(osv.Model):
	...
	def unlink(self , cr , uid , ids , context=None):
		...
		return super(hr_employee_medal , self).unlink(cr , uid , ids , context)

	_sql_constraints = [
        ('model_uniq', 'unique(employee_id,medal_id)', _('Employee connot take gift more than one!!')),
    ]
