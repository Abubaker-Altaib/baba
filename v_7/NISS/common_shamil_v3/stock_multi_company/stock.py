    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            if picking.state not in ['draft']:
                raise osv.except_osv(_('Error'), _('You cannot remove this picking which is not in draft state.'))
...
class stock_move(osv.Model):
    ...
    def _check_picking_type(self, cr, uid, ids, context=None):
        for move in self.browse(cr, uid, ids, context=context):
            # You can not send or recieve goods to the same location
            if move.state != 'draft' and move.location_id.id == move.location_dest_id.id:
                return False
        return True

    _constraints = [
        (_check_picking_type, 
            'You can not send or recieve goods to same location',
            ['location'])]
...
class stock_warehouse(osv.Model):
    ...
    _sql_constraints = [
       	    ('name_uniq', 'unique(name)', 'Warehouse name must be unique !'),
    ]
...
class stock_journal(osv.osv):
    ...
    _sql_constraints = [
       	    ('name_uniq', 'unique(name)', 'Stock Journal name must be unique !'),
    ]
