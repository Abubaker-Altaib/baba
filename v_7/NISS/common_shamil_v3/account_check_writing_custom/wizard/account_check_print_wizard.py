...
class account_check_print_wizard(osv.osv_memory):
    ...
    def _get_state(self, cr, uid, context=None):
        ...
        if not ids: return 'draft'
        voucher = voucher_pool.browse(cr, uid, ids, context=context)
        result= voucher and ((voucher.move_id.state == 'draft' and not voucher.chk_seq and 'nothing') or 
                            (voucher.move_id.state == 'draft' and 'unpost') or (voucher.chk_seq and 'printed' or 'draft'))
        return result
    def _get_msg(self, cr, uid, context=None): 
        ...
        return (move not in ('draft','completed','closed') and _("Your payment's move is not posted!")) or (chk_no and _("This Payment has already been paid with check:%s")%(chk_no) or \
            ok)
    ...
    _columns =  {
        ...
        'new_no': fields.char('Update Check Number', help='Enter new check number here if you wish to update'),
        
        'nxt_chk_no': fields.char('Next Check Number', size=64, help='Next check number'),
        
        'new_chk_no': fields.char('Update Check Number', help='Enter new check number here if you wish to update'),
