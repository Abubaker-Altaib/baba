...
class hr_evaluation_interview(osv.Model):
	...
	evaluation_id .
        @param phase_id: Id of the pahse
...
class hr_evaluation(osv.osv):
    ...
    """Inherits hr_evaluation.evaluation to add fiels and functions that handle employee's evaluation .
    """
	...
	Method that over writes button_plan_in_progress (mainly it adds the phase_id to the 
        enterview record) it goes through the phases of the evaluation and for each phase it
        gets the interviewers and for each one of them it creates an enterview records and sets  
        the phase in the wait state if its not it olse sends email notification to the interviewers 
