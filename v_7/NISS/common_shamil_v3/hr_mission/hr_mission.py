...
#HR Operation Motharek
#----------------------------------------
class operation_motharek(osv.osv):
    _name = "hr.operation.mth"
    _description = "Operation Motharek"

    _columns = {
        'name': fields.char('Motharek name', size=256,required=True),
    }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if 'name' in vals:
            vals['name'] = vals['name'].strip()
        if not vals['name']:
            raise osv.except_osv(_('Warning!'),_('The Operation Motharek Name Must Not be space'))
        return super(operation_motharek, self).create(cr, uid, vals, context=context)
...
class employee_mission(osv.osv):
    ...
    _columns = {
            ...
            'voucher_state': fields.related('voucher_id', 'state', type='char', string='Voucher State',readonly=True),
		    'fees_voucher_id' :fields.many2one('account.voucher','Fees Voucher',readonly=True),
            'fees_voucher_state': fields.related('fees_voucher_id', 'state', type='char', string='Fees Voucher State',readonly=True),
            ...
            'mth_id':fields.many2one('hr.operation.mth','Motharek'), 
    ...
    def onchange_date(self, cr, uid, ids, start_date, end_date, days_no, field, context=None):
        ...
        if field == 'start_date':
            if dt_from and days_no is not None:
                dt_time=dt_from + datetime.timedelta(days_no)
    ...
    def operation_duration_computation(self, cr, uid, ids,context=None):
        ...
        """
        hr_setting = self.pool.get('res.company')
        config_ids=hr_setting.search(cr,uid,[])
        config_browse=hr_setting.browse(cr, uid, config_ids)[0]
        settings=config_browse.operation_service
        for rec in self.browse(cr,uid,ids,context=context):
            settings=rec.company_id.operation_service
...
class employee_mission_line(osv.osv):
    ...
    _columns = {
        ...
        'state': fields.related('emp_mission_id','state',type="char", string="mission State"),
    ...
    def check_employee_mission(self, cr, uid, ids, context={}):
        ...
        append_obj = self.pool.get('hr.append')

        for line in self.browse(cr,uid,ids,context=context):

            # CHECK 1:- check If the piriod of mission intersect with piriod of mission 
            active_mission_ids1=self.search(cr,uid,[('start_date','<=',line.start_date),('end_date','>=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('type','=',line.type)],context=context)
            active_mission_ids2=self.search(cr,uid,[('start_date','>=',line.start_date),('end_date','<=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('type','=',line.type)],context=context)
            active_mission_ids3=self.search(cr,uid,[('start_date','<=',line.start_date),('end_date','>=',line.start_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('type','=',line.type)],context=context)
            active_mission_ids4=self.search(cr,uid,[('start_date','<=',line.end_date),('end_date','>=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('type','=',line.type)],context=context)
            active_mission_ids=list(set(active_mission_ids1)|set(active_mission_ids2)|set(active_mission_ids3)|set(active_mission_ids4))
            if active_mission_ids and line.type == '1':
                raise osv.except_osv(_('ERROR'),_('Sorry This Employee Is currently in a mission!(%s)')%(line.employee_id.name))
                return False
            elif active_mission_ids and line.type == '2':
                raise osv.except_osv(_('ERROR'),_('Sorry This Employee Is currently in a Operation!(%s)')%(line.employee_id.name))
                return False
            elif active_mission_ids and line.type == '3':
                raise osv.except_osv(_('ERROR'),_('Sorry This Employee Is currently in a Trasmission!(%s)')%(line.employee_id.name))
                return False

            # CHECK 2:- check If the piriod of mission intersect with piriod of absence
            absence_idss1 = absence_obj.search(cr, uid, [('date_from','>=',line.start_date),
                                ('date_to','<=',line.end_date),('employee_id','=',line.employee_id.id),('state','in',['confirmed'])],
                                context=context)
            absence_idss2 = absence_obj.search(cr, uid, [('date_from','<=',line.start_date),
                                ('date_to','>=',line.end_date),('employee_id','=',line.employee_id.id),('state','in',['confirmed'])],
                                context=context)
            absence_idss3 = absence_obj.search(cr, uid, [('date_from','<=',line.end_date),
                                ('date_to','>=',line.end_date),('employee_id','=',line.employee_id.id),('state','in',['confirmed'])],
                                context=context)
            absence_idss4 = absence_obj.search(cr, uid, [('date_from','<=',line.start_date),
                                ('date_to','>=',line.start_date),('employee_id','=',line.employee_id.id),('state','in',['confirmed'])],
                                context=context)
            absence_idss=list(set(absence_idss1)|set(absence_idss2)|set(absence_idss3)|set(absence_idss4))
            if absence_idss:
                raise osv.except_osv(_('ERROR'),_('Sorry This Employee Is currently in a Absence (%s)!')%(line.employee_id.name))
                return False

            # CHECK 3:- check If the piriod of mission intersect with piriod of holiday
            holiday_idss1 = holiday_obj.search(cr, uid, [('date_from','>=',line.start_date),
                                ('date_to','<=',line.end_date),('employee_id','=',line.employee_id.id),('state','in',['validate'])],
                                context=context)
            holiday_idss2 = holiday_obj.search(cr, uid, [('date_from','<=',line.start_date),
                                ('date_to','>=',line.end_date),('employee_id','=',line.employee_id.id),('state','in',['validate'])],
                                context=context)
            holiday_idss3 = holiday_obj.search(cr, uid, [('date_from','<=',line.end_date),
                                ('date_to','>=',line.end_date),('employee_id','=',line.employee_id.id),('state','in',['validate'])],
                                context=context)
            holiday_idss4 = holiday_obj.search(cr, uid, [('date_from','<=',line.start_date),
                                ('date_to','>=',line.start_date),('employee_id','=',line.employee_id.id),('state','in',['validate'])],
                                context=context)
            holiday_idss=list(set(holiday_idss1)|set(holiday_idss2)|set(holiday_idss3)|set(holiday_idss4))
            if holiday_idss:
                raise osv.except_osv(_('ERROR'),_('Sorry This Employee Is currently in a holiday (%s)!')%(line.employee_id.name))
                return False

            # CHECK 4:- check If the piriod of mission intersect with piriod of training 
            training_idss1=training_obj.search(cr,uid,[('start_date','<=',line.start_date),('end_date','>=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id)],context=context)
            training_idss2=training_obj.search(cr,uid,[('start_date','>=',line.start_date),('end_date','<=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id)],context=context)
            training_idss3=training_obj.search(cr,uid,[('start_date','<=',line.start_date),('end_date','>=',line.start_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id)],context=context)
            training_idss4=training_obj.search(cr,uid,[('start_date','<=',line.end_date),('end_date','>=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id)],context=context)
            training_idss=list(set(training_idss1)|set(training_idss2)|set(training_idss3)|set(training_idss4))
            if training_idss:
                raise osv.except_osv(_('ERROR'), _('Sorry This Employee Is currently in Training (%s)!') % (line.employee_id.name))
                return False

            # CHECK 5:- check If the piriod of mission intersect with piriod of append
            append_idss1=append_obj.search(cr,uid,[('start_date','<=',line.start_date),('end_date','>=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('state','=','confirm')],context=context)
            append_idss2=append_obj.search(cr,uid,[('start_date','>=',line.start_date),('end_date','<=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('state','=','confirm')],context=context)
            append_idss3=append_obj.search(cr,uid,[('start_date','<=',line.start_date),('end_date','>=',line.start_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('state','=','confirm')],context=context)
            append_idss4=append_obj.search(cr,uid,[('start_date','<=',line.end_date),('end_date','>=',line.end_date),
                                                   ('id','!=',line.id),('employee_id','=',line.employee_id.id),('state','=','confirm')],context=context)
            append_idss=list(set(append_idss1)|set(append_idss2)|set(append_idss3)|set(append_idss4))
            #if append_idss:
            #    raise osv.except_osv(_('ERROR'), _('Sorry This Employee Is currently in Append (%s)!') % (line.employee_id.name))
            #    return False
        return True
    ...
    def onchange_employee(self, cr, uid, ids, emp_id,mission_id,days ,context=None):
        """
