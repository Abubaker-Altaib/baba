...
class account_budget_update(osv.osv_memory):
    ...
    _columns = {
        ...
        'period_id': fields.many2one('account.period', 'Period', type='many2one' ),
    }
    ...
    def update(self, cr, uid, ids, context={}):
        ...
        context.update({'update_buget':True})
        domain = [('analytic_account_id', 'in', [analytic_account.id for analytic_account in wiz.analytic_account_ids])]
        if wiz.period_id: domain.append(('period_id','=',wiz.period_id.id))
