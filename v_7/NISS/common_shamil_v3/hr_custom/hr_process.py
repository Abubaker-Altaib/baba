...
class hr_employee_delegation(osv.Model):
    ...
    _constraints = [
        (_check_date, 'You can not have 2 delegation that overlaps on same day!', ['start_date','end_date']),
    ]
    _sql_constraints = [ ('date_check2', "CHECK ( start_date <= end_date)", "The start date must be anterior to the end date."),  
    ] 
