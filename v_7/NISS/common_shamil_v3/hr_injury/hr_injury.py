...
class hr_config_settings_inherit(osv.osv_memory):
    ...
    def get_default_treatment_account_id(self, cr, uid, ids, context=None):
        ...
        """
        treatment_account = self.pool.get("ir.config_parameter").get_param(
            cr, uid, "treatment_account_id", context=context)
    ...
    def set_treatment_account_id(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            config_parameters.set_param(
                cr, uid, "treatment_account_id", record.treatment_account_id.id or False, context=context)
...
class hr_injury(osv.Model):
    ...
    def _compute_emp_payroll(self, cr, uid, ids, field_name, arg, context={}):
        ...
        for pay in self.browse(cr, uid, [pay_id], context):
            ...
            total_amount = 0.0
            amount = 0.0
            if pay.inability_type.allowances_id:
                allow_ids = [pay.inability_type.allowances_id.id]
                total_amount = payroll_obj.read_allowance_deduct(
                    cr, uid, pay.name.id, allow_ids, 'allow')
                started_section_ids = salary_degree_obj.search(cr, uid, [(
                    'payroll_id', '=', pay.name.payroll_id.id), ('id', '=', pay.name.degree_id.id)], context=context)
                if started_section_ids:
                    started_section = salary_degree_obj.browse(
                        cr, uid, started_section_ids, context=context)[0]
    ...
    _columns = {
        ...
        'accident_address': fields.text("Accident Address"),
        'work_type': fields.text("Work Type"),
        'first_witness': fields.char("First witness", size=100, required=False),
        'second_witness': fields.char("Second witness", size=100, required=False),
        ...
        # required=True
        'date': fields.date("Transfer Date", required=False),
    ...
    def onchange_employee(self, cr, uid, ids, emp_id, context={}):
        ...
        employee_domain = emp_obj._get_default_employee_domain(
            cr, uid, ids, contractors, employee, recruit, trainee)
    ...
    def create(self, cr, uid, vals, context={}):
        ...
        @return: Integer ID of the created recordcode
        """
        salary_degree_obj = self.pool.get('hr.salary.degree')
        payroll_obj = self.pool.get('payroll')
        compensation_id = super(osv.osv, self).create(
            ...
            if rec.acc_number:
                if rec.acc_number.state == 'draft':
                    self.pool.get('account.voucher').unlink(
                        cr, uid, [rec.acc_number.id])
                elif rec.acc_number.state == 'cancel':
                    self.write(cr, uid, ids, {'acc_number': False})
                else:
                    raise osv.except_osv(_('warning'), _(
                        'There is a voucher releted to this record, you must cancel it before set the record to draft'))
        ...
        for record in self.browse(cr, uid, compensation_id, context=context):
            ...
            if record.inability_type.allowances_id:
                ...
                allows_amount = payroll_obj.read_allowance_deduct(
                    cr, uid, record.name.id, allow_ids, 'allow')
...
class hr_injury_type(osv.Model):
    ...
    _columns = {
        ...
        'by_type': fields.one2many('hr.injury', 'injury_type', 'By Type'),
        'by_reason': fields.one2many('hr.injury', 'injury_reason', 'By Reason'),
    }
    ...
    _sql_constraints = [
        ('injury_type_code_uniqe', 'unique(code)', 'you can not create same code !'),
        ('injury_type_name_uniqe', 'unique(name)', 'you can not create same name !')
    ]

    def unlink(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context):
            if rec.by_type or rec.by_reason:
                raise osv.except_osv(_('ValidateError'), _("this record is linked or ther record"))
        super(hr_injury_type, self).unlink(cr, uid, ids, context=context)
...
class hr_inability_type(osv.Model):
    ...
    _columns = {
        ...
        'injury_id': fields.one2many('hr.injury', 'inability_type', 'Injury'),
    }
    def _check_spaces(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context=context):
            if rec.name and (len(rec.name.replace(' ','')) <= 0):
                raise osv.except_osv(_('ValidateError'), _("Inability Type must not be spaces"))
            if rec.code and (len(rec.code.replace(' ','')) <= 0):
                raise osv.except_osv(_('ValidateError'), _("code must not be spaces"))
            if self.search(cr, uid, [('name','=',rec.name),('id','!=',rec.id)],  context=context):
                raise osv.except_osv(_('ValidateError'), _("you can not create same code !"))
    ...
    def unlink(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context):
            if rec.injury_id:
                raise osv.except_osv(_('ValidateError'), _("this record is linked or ther record"))
        super(hr_inability_type, self).unlink(cr, uid, ids, context=context)

    _constraints = [
        (_check_spaces, '', ['code','name'])
    ]
    _sql_constraints = [
        ('inability_typecode_uniqe', 'unique(code)', 'you can not create same code !'),
        ('inability_typename_uniqe', 'unique(name)', 'you can not create same name !')
    ]
