...
class common_report_header(object):
    ...
    def _sum_debit(self, period_id=False, journal_id=False):
        ...
        if not (period_id and journal_id):
            return 0.0
        self.cr.execute('SELECT SUM(debit) FROM account_move_line l , account_move m'
                        'WHERE l.move_id = m.id AND m.state <> %s and period_id IN %s AND journal_id IN %s ' + self.query_get_clause + ' ',
                        ('reversed',tuple(period_id), tuple(journal_id)))
    ...
    def _sum_credit(self, period_id=False, journal_id=False):
        ...
        if not (period_id and journal_id):
            return 0.0
        self.cr.execute('SELECT SUM(credit) FROM account_move_line l , account_move m '
                        'WHERE l.move_id = m.id AND m.state <> %s period_id IN %s AND journal_id IN %s '+ self.query_get_clause+'',
                        ('reversed',tuple(period_id), tuple(journal_id)))
        return self.cr.fetchone()[0] or 0.0
