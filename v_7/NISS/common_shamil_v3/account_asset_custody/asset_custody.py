...
class asset_custody(osv.osv):
	...
	def copy(self, cr, uid, ids, default=None, context=None):
		"""
		Override copy function to stop copy in recived state.

		@param default : default vals dict
		@return: super copy() method
		"""
		for rec in self.browse(cr, uid, [ids], context):
			if rec.state in ['recived']: 
				raise osv.except_osv(_('Warning!'),_('You cannot duplicate record in recived state.'))
		return super(asset_custody, self).copy(cr, uid, ids, default, context)
	...
	'history_line':fields.one2many('history.custody.line','custody_id', 'History Line'),
		...
		'serial_no': fields.related('asset_line', 'serial_no', type='char', string='Serial', readonly=True),
				...
				for custody_id in custody_line_ids:
					...
					for custody_h in custody_h:
						if custody.show_released==False:
							if custody_h.state_rm =='assigned':
		...
		for order in self.browse(cr, uid, ids, context=context):
			...
			if order.type=="request":
				...
				if  not order.stock:  # DATA MIGRATION
				 ...
				 # UPDATE VALUES IF  CUSDODIES CREATED FROM STOCK
				if  order.stock: 
					for custody_line in order.asset_line:
						if  custody_line.asset_id:
							# UPDATE ASSET ITEM
							asset_id= custody_line.asset_id.id
							asset_id = asset_obj.write(cr, uid,asset_id, {
...
class account_asset_asset(osv.osv):
	...
	'department_id_rel': fields.related('employee_id', 'department_id', type='many2one', relation='hr.department', string='Department', readonly=True),
