...
class account_journal(osv.Model):
    ...
    def _check_company(self, cr, uid, ids):
        """
        Constrain method to asure that the journal sequence comany and 
    ...
    _constraints = [(_check_company, 'Journal company and Entry sequence company do not match.', ['company_id'])]
...
class account_voucher(osv.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        if isinstance(ids, (int, long)):
                    ids = [ids]
        reads = self.read(cr, uid, ids, ['number', 'state'], context=context)
        colums = self._columns['state'].selection
        res = []
        for record in reads:
            name = '-'
            for c in colums:
               if c[0]==record['state']: name = c[1]
               break
            if record['number']:
    ...
    def onchange_journal_id(self, cr, uid, ids, journal, pay_journal, line_ids, tax_id, partner_id, date, amount, ttype, company_id, pay_now, context=None):
        """
        ...
        account_id = ttype in ('purchase','payment') and \
                    ...
                    or (pay_now == 'pay_now' and (journal_obj and journal_obj.default_debit_account_id.id) or \
                       (partner_obj and partner_obj.property_account_payable and partner_obj.property_account_payable.id))
    ...
    def onchange_part(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, pay_now, context=None):
        journal = self.pool.get('account.journal').browse(cr, uid, journal_id, context=context)
        if journal.id != 0:
           if journal.type not in ('cash', 'bank') and pay_now=='pay_now':
              return {}
        return super(account_voucher,self).onchange_partner_id(cr, uid, ids, partner_id, \
                                journal_id, price, currency_id, ttype, date, context=context)
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        ...
        default = {
                'value': journal.type in ('cash', 'bank') and \
                        {'line_ids':[], 'line_dr_ids':[], 'line_cr_ids':[], 'pre_line': False, 'currency_id':currency_id, 'payment_rate_currency_id': currency_id, 'payment_rate': 1} or \
                        {'pre_line': False, 'currency_id':currency_id, 'payment_rate_currency_id': currency_id,'payment_rate': 1},
            }
        if ttype == 'purchase': 
        ...
        if journal.type not in ('cash', 'bank') or context.get('ttype', False):
            return default
        ...
        for line in moves:
            ...
            default['value']['pre_line'] = (ttype == 'payment' and len(default['value']['line_cr_ids']) > 0) or (ttype == 'receipt' and len(default['value'].get('line_dr_ids',[])) > 0)
    ...
    def action_payment_term_create(self, cr, uid, voucher, context=None):
        ...
        journal_id = voucher.pay_journal_id or voucher.journal_id
    ...
    def compute_tax(self, cr, uid, ids, context=None):
        '''
        Calculate Voucher Amount before taxs, Tax Amount and Total Voucher Amount with Taxs,
    ...
    def action_move_line_create(self, cr, uid, ids, vals={}, context=None):
        ...
        period_pool = self.pool.get('account.period')
        ml_id=[]
#-----ckeck payment total is greater than zero
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            if voucher.type in ('payment','receipt'):
                if voucher.amount <= 0.00:
                    raise osv.except_osv(_('Entry Error!'), _('Payment Total is invalid!'))
            if voucher.move_id:
                continue
    ...
    def create(self, cr, uid, vals, context={}):
        ...
        """
        journal_id = vals.get('type') in ('receipt') and  vals.get('pay_journal_id' ) or vals.get('journal_id') 
        if journal_id:
    ...
    def onchange_amount(self, cr, uid, ids, amount, rate, partner_id, journal_id, currency_id, ttype, date, payment_rate_currency_id, company_id, context=None):
        """
        Onchange method to resend the company ID based on user company
    ...
    def _required_line_id(self, cr, uid, ids, context=None):
        if self.search(cr, uid,[('id', 'in', ids),('line_ids', '=', False),('state', '!=', 'draft')], context=context):
            return False
        return True

    def _amount_check(self, cr, uid, ids, context=None):
        voucher_ids = self.search(cr, uid, [('state','!=','draft'),('id','in',ids)], context=context)
...
class account_voucher_line(osv.Model):
    ...
    def _account_line_id(self, cr, uid, ids, context=None):
        ...
        if obj.voucher_id.operation_type:
            if obj.account_id.id == obj.voucher_id.pay_journal_id.default_credit_account_id.id:
                return False
        return True
    ...
    def onchange_account(self, cr, uid, ids, account_id, context={}):
        ...
        if account_id:
            ...
            return {'value': {'account_balance': account.balance}}
        return {}
