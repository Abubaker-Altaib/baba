...
class hr_commision(osv.Model):
    ...
    _columns = {
        ...
        #                         'Type'),
        'type': fields.many2one('hr.injury.type', "Injury Type", domain="[('type','!=','injury_reason')]"),
        ...
        'invoice_no': fields.integer("Invoice No", required=False),
        ...
        'state': fields.selection([('draft', 'Draft'), ('refuse', 'Refused'), ('validate3', 'Approved'), ('cancel', 'Cancelled'), ('transfer', 'Transfered')],
                                  'State', readonly=True),
        'status_mode': fields.selection([('open', 'Open'), ('close', 'Closed'), ('temp_close', 'Temporarily Closed')], 'Status Mode'),
        'inability_type': fields.many2one('hr.inability.type', "Inability Type",),
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        if context is None:
            context={}
        res = super(hr_commision, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
    ...
    def unlink(self, cr, uid, ids, context=None):
        for rec in self.browse(cr, uid, ids, context):
            ...
            if rec.injury_id:
                raise osv.except_osv(_('ValidateError'), _("this record is linked to other record"))
        ...
        for record in self.browse(cr, uid, compensation_id, context=context):
            ...
            if record.inability_percentage <= 0.0:
                raise osv.except_osv(_('ERROR'), _(
                    'Inability percentage less than or equal zero '))
            if record.inability_type.allowances_id:
                allow_ids = [record.inability_type.allowances_id.id]
                allows_amount = payroll_obj.read_allowance_deduct(
                    cr, uid, record.employee_id.id, allow_ids, 'allow')
                total_amount = allows_amount * record.inability_percentage
            else:
                raise osv.except_osv(_('ERROR'), _(
                    'The allowance for inability type is not existed'))
            self.write(cr, uid, compensation_id, {
                       'inability_amount': total_amount, }, context=context)
        return True
        ...
        for rec in self.browse(cr,uid,ids,context=context):
            ...
            if rec.inability_acc_number :
                    if rec.inability_acc_number.state == 'draft' :
                        ...
                        self.write(cr,uid,ids,{'inability_acc_number':False, 'transfer': False, 'compensation_transfer': False})
                    else : 
                        raise osv.except_osv(_('warning') , _('There is a voucher releted to this record, you must cancel it before set the record to draft'))
    ...
    def transfer_rec(self, cr, uid, ids, context={}):
        ...
        #config_ids = hr_setting.search(cr, SUPERUSER_ID, [('company_id','=',injury.employee_id.company_id.id)],order='id desc')
        ...
        if not injury.inability_acc_number:
            if injury.inability_amount > 0.0:  
                #if config_browse and config_browse[0].hr_journal_id and injury.department_id.analytic_account_id and config_browse[0].treatment_account_id:
                if company.hr_journal_id and injury.department_id.analytic_account_id and company.treatment_account_id:
                   #compensation_dict={'account_id': config_browse[0].treatment_account_id.id,'amount': data.inability_amount,}
                   compensation_dict={'account_id': company.treatment_account_id.id,'amount': data.inability_amount,}
                   lines2.append(compensation_dict) 
                   voucher=self.pool.get('payroll').create_payment(cr, uid, ids,{'reference':reference,'lines':lines2},context=context)
                   emp_injury_obj.write(cr, uid, [emp_injury_id], {'inability_acc_number':voucher, 'compensation_transfer':True, 'state':'transfer', 'transfer': True }, context=context)
                else:
...
#----------------------------------------
# injury types inherit
#----------------------------------------
class hr_injury_type(osv.Model):
    _inherit = 'hr.injury.type'
    _columns = {
    'company_id': fields.many2one('res.company','company'),
        }
...
#----------------------------------------
# inability types inherit
#----------------------------------------
class hr_inability_type(osv.Model):
    _inherit = 'hr.inability.type'
    _columns = {
    'company_id': fields.many2one('res.company','company'),
        }
...
class hr_injury_movement(osv.Model):
    _name = 'hr.injury.movement'
    _columns = {
        'name': fields.char('Movement Of Correspondence', size=256),
        }
...
class hr_injury(osv.Model):
    ...
    _columns = {
        ...
        'company_id': fields.many2one('res.company','company'),
        'injury_move_id': fields.many2one('hr.injury.movement','Movement Of Correspondence'),
        'degree_id': fields.related('name', 'degree_id', string='Degree', type='many2one', relation='hr.salary.degree', readonly=True, store=True),
        'reference': fields.char('Reference'),
        'attached': fields.char('Attached'),
        }
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context=None, toolbar=False, submenu=False):
        ...
        if context is None:
            context={}
        res = super(hr_injury, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
        emp=emp_obj.search(cr, uid, [('user_id','=',uid)])
        employee = emp_obj.browse(cr, uid, emp and emp[0] or 0)
        for cat in (emp and employee.category_ids or []):
            if cat.belong_to:
        ...
        if belong_to:
            ...
            for node in doc.xpath("//field[@name='name']"):
                if belong_to == 'officer_affairs':
                    ...
                    node.set('string', _('Soldier'))
            for node in doc.xpath("//label[@for='name']"):
                if belong_to == 'officer_affairs':
                    node.set('string', _('Officer'))
                elif belong_to == 'soldier_affairs':
                    node.set('string', _('Soldier'))
                    ...
                    node.set('string', _('Soldier'))
            for node in doc.xpath("//separator[@string='employee']"):
                if belong_to == 'officer_affairs':
                    node.set('string', _('Officer'))
                elif belong_to == 'soldier_affairs':
                    node.set('string', _('Soldier'))
