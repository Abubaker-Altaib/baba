...
class hr_promotions_candidates(osv.osv_memory):
    ...
    def promotions_candidates_dict(self,cr,uid,ids,context={}):
                            ...
                            for new_degree in salary_degree_obj.browse(cr,uid,new_degree_id,context=context):
                                for employee in employee_obj.browse(cr,uid,employee_ids,context=context):
                                    ...
                                    if not employee.promotion_date:
                                        print"************************year out of range***********************employee.id",employee.id
                                    ...
                                    if days <= c.margin :
                                        emp_degree_dict = {
                                             ...
                                             'service_state': employee.service_state_id.name,
                                             'employee': employee,
                                                         }
                                        emp_list.append(emp_degree_dict)

	    return emp_list

    def print_candidates(self,cr,uid,ids,context={}):

        emp_list = self.promotions_candidates_dict(cr,uid,ids,context=context)
        ...
        return {
            ...
            'report_name': 'promotion_candidate_report',
            'datas':datas,
        }
