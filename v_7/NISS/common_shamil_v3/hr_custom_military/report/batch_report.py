...
class batch(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        self.count = 0
        self.context = context
        super(batch, self).__init__(cr, uid, name, context)
        self.h_deps_ids = []
    ...
    def _get_all_len(self, data):
        parent_batch = data['form']['parent_batch']
        batch = data['form']['batch']
        placement_batch = data['form']['placement_batch']
        ...
        if parent_batch:
            parent_batch = parent_batch[0]
            if clouses:
                clouses += " and emp.parent_batch="+str(parent_batch)
            if not clouses:
                clouses = "emp.parent_batch="+str(parent_batch)
        
        if batch:
            batch = batch[0]
            if clouses:
                clouses += " and emp.batch="+str(batch)
            if not clouses:
                clouses = "emp.batch="+str(batch)
            
        if placement_batch:
            placement_batch = placement_batch[0]
            if clouses:
                clouses += " and emp.placement_batch="+str(placement_batch)
            if not clouses:
                clouses = "emp.placement_batch="+str(placement_batch)
        ...
        query = """select parent_batch.name as parent_batch_name,batch.name batch_name,placement_batch.name as placement_batch_name,emp.otherid,emp.name_related,
...
report_sxw.report_sxw('report.hr.batch.report', 'hr.employee',
                      'addons/hr_custom_military/report/batch_report.mako', parser=batch, header=False)
