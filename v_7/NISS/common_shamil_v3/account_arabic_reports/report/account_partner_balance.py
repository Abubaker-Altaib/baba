    ...
    def account_partners(self, account, init):
        ...
        if self.init_query:
            self.cr.execute(
                ...
                "FROM   (SELECT p.id, l.move_id, p.name AS name,p.code AS code, "\
                    ...
                    "GROUP BY l.move_id,p.id, p.name,p.code " \
                    ") as result "\
...
###############
        ...
        if self.init_query:
            ...
            self.cr.execute(
                    "SELECT p.code,p.name , 0 as debit , 0 as credit , l.partner_id, COALESCE(sum(debit-credit), 0.0) AS init_bal \
                     ...
                     GROUP BY l.partner_id, p.name,p.code", param)
