    ...
    def lines(self, data):
        ...
        if self.type_selection == 'total':
            self.cr.execute("SELECT COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0.0) AS debit, MIN(m.ref) AS ref,\
             ...
             self.cr.execute("SELECT l.id, COALESCE(l.amount_currency,0) as amount_currency, COALESCE(l.credit,0) as credit, COALESCE(l.debit,0) as debit, l.amount_currency|| ' ' ||c.symbol as currency,  l.name as label, m.ref, m.name as move, l.date \
...
report_sxw.report_sxw('report.account.account.statement.detailed', 'account.account', 'addons/account_arabic_reports/report/account_statement_detailed.rml', parser=account_statement, header=False)
