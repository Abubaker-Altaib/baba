...
class stock_return_picking_memory(osv.osv_memory):
    ...
    _columns = {
        ...
        'exchange_wizard_id' : fields.many2one('stock.exchange.return.picking', string="Wizard"),
    }

    def _check_product_qty(self, cr, uid, ids):
        for record in self.browse(cr, uid, ids):
            if record.quantity > record.move_id.product_qty:
                raise osv.except_osv(_('Error !'), _('The Quantity cannot be more than original quantity.'))
                return False
            return True
    ...
    def _check_product_qty_positive(self, cr, uid, ids):
        for record in self.browse(cr, uid, ids):
            if record.quantity <= 0:
                raise osv.except_osv(_('Error !'), _('The Quantity must be more than zero.'))
                return False
            return True

    _constraints = [
                 (_check_product_qty, '',['quantity']),
                 (_check_product_qty_positive, '',['quantity']),]

stock_return_picking_memory()


class stock_exchange_return_picking(osv.osv_memory):
    _name = 'stock.exchange.return.picking'
    _description = 'Return Picking'
    _columns = {
        'product_return_moves' : fields.one2many('stock.return.picking.memory', 'exchange_wizard_id', 'Moves'),
    ...
    def _check_product_qty(self, cr, uid, ids):
        for record in self.browse(cr, uid, ids):
            for line in record.product_return_moves:
                if line.quantity > (line.move_id.product_qty - line.move_id.exchange_line_id.returned_qty):
                    raise osv.except_osv(_('Error !'), _('The Quantity cannot be more than original quantity.'))
                    return False
            return True
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        res = super(stock_exchange_return_picking, self).default_get(cr, uid, fields, context=context)
        exchange_id = context and context.get('active_id', False) or False
        pick_list = [pick.id for pick in self.pool.get('exchange.order').browse(cr, uid, exchange_id).picking_ids]
        record_id = pick_list[len(pick_list)-1]
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        if context is None:
            context = {}
        res = super(stock_exchange_return_picking, self).view_init(cr, uid, fields_list, context=context)
        exchange_id = context and context.get('active_id', False)
        if exchange_id:
            pick_list = [pick.id for pick in self.pool.get('exchange.order').browse(cr, uid, exchange_id).picking_ids]
            record_id = pick_list[len(pick_list)-1]
            pick_obj = self.pool.get('stock.picking')
            ...
            if m.state == 'done':
                ...
                for rec in m.move_history_ids2:
                    ...
                    if rec.location_dest_id.id == m.location_id.id \
                        and rec.location_id.id == m.location_dest_id.id and rec.state != 'cancel' and rec.type == 'in':
...
#        Create new picking for returned products
        ...
        new_picking = pick_obj.copy(cr, uid, pick.id, {
                                        'name': _('%s-%s-return') % (new_pick_name, pick.exchange_id.name),
                                        'move_lines': [], 
                                        'state':'draft', 
                                        'type': new_type,
                                        'date':date_cur, 
        ...
        for v in val_id:
            ...
            if new_qty:
                ...
                #move.exchange_line_id.write({'returned_qty':returned_qty1 + new_qty})
        ...
        dummy, menu_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'vehicles_maintenance', 'menu_action_picking_tree4')
        dummy, form_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'vehicles_maintenance', 'view_picking_in_form')
        ...
        '''return {
            ...
            #'context':context,
            #'action': action,
            #'model': model_list.get(new_type, 'stock.picking'),
            'id': menu_id,
            #'action':action,
            'view_type':'form',
            #'type':'ir.actions.act_window',
