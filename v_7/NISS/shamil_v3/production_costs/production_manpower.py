...
class mrp_production_manpower(osv.osv):
    ...
    def _employee_cost(self, cr, uid, ids, name, args,context=None):
        """
	Method for functional fields that computes thr amount of cost employee.
	...
	for rec in self.browse(cr, uid, ids, context=context):
            allow_dict=payroll_obj.allowances_deductions_calculation(cr,uid,rec.start_date,rec.employee_id,{},[], False,[])
            result[rec.id]=(allow_dict['total_allow']+rec.employee_id.bonus_id.basic_salary)/240*rec.production_duration
        return result
    ...
    _columns = {
        ...
        'production_id': fields.many2one('mrp.production', 'Related production order'),
      	'employee_cost': fields.function(_employee_cost, string='Cost', type='float', method=True, readonly=True),
    }
