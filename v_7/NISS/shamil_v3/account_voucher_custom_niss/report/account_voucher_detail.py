...
class report_voucher_detail(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(report_voucher_detail, self).__init__(cr, uid, name, context)
    ...
    def get_lines(self, voucher):
        ...
        if voucher.type in ('payment','receipt'):
            ...
            type = voucher.line_ids and voucher.line_ids[0].type or False
            for move in voucher.line_ids:
                    res = {'ref':  move.name, 'name': move.name, 'account_id': move.account_id.name, 'amount': move.amount,'account_analytic_id':move.account_analytic_id.name,'partner_id':move.res_partner_id.name
,'partner_code':move.res_partner_id.code,'code':move.account_id.code, 'line_id':move.id}
                    result.append(res)
        return result
...
report_sxw.report_sxw('report.account.voucher.details', 'account.voucher', 'addons/account_voucher_custom/report/account_voucher_details.rml', parser=report_voucher_detail,header='external' )
