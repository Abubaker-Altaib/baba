...
class account_close_period(osv.osv_memory):
    ...
    def get_moves(self, cr, uid, ids, context=None):
        ...
        if not data['period_id'] and not data['date_from'] and not data['date_to']:
            raise osv.except_osv(_('Error!'), _('Sorry , Please select period or date'))
        domain=[('state','in',('completed','closed'))]
        if data['period_id']:
