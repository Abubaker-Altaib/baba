...
class account_voucher_line(osv.osv):
    ...
    def _check_custody_end_date(self, cr, uid, ids, context=None):
        """ Checks if custody date bigger than voucher date.
        @return: True or False
        """
        for line in self.browse(cr, uid, ids, context=context):
            if line.custody_end_date and line.custody_end_date < line.date:
                return False
        return True
    ...
    _columns = {
		...
		'second_partner_id' : fields.many2one('res.partner','Second Partner'),
        	'custody_end_date':fields.date('Custody End Date'),
	   }
    _defaults = {
         'seq': 0,
    ...
    _constraints = [
        (_check_custody_end_date, 'Custody End Date must be bigger than voucher date',['Custody End Date'])]
    ...
    def on_change_custody(self, cr, uid, ids,res_partner_id,custody,partner_id,context=None):
        """ This Function Dupliacted The Partner In Line if it custody and the user belong to OC Group """

        partner = res_partner_id
        has_group = self.pool.get('res.users').has_group(cr,uid,'account_voucher_custom_niss.group_account_voucher_niss_oc')
        
        if has_group and custody : 
           partner = partner_id
        return { 'value' : { 'res_partner_id' : partner }   }
...
class account_move_line(osv.osv):
    ...
    _columns = {
		...
		'voucher_line_id' : fields.many2one('account.voucher.line','voucher line'),
                ...
                'custody_end_date':fields.related('voucher_line_id', 'custody_end_date', type='date', string='Custody End Date',),
...
class account_move(osv.osv):
    ...
    def _check_move_date(self, cr, uid, ids, context=None):
        for move in self.browse(cr, uid, ids, context=context):
            if move.date > str(datetime.date.today()):
                return False
        return True

    _constraints = [(_check_move_date, 'You Cann\'t  create Account Move in future date!', ['Date']),] 
...
class account_voucher(osv.osv):
    ...
    def _check_permission_uniq(self, cr, uid, ids, context=None):
        for voucher in self.browse(cr, uid, ids, context=context):
            if not voucher.permission:
                return True
            if ' ' in voucher.permission:
                raise osv.except_osv(_('Error!'), _('You can\'t add permission with space character'))
    ...
    def _check_pay_type(self, cr, uid, ids, context=None):
        for voucher in self.browse(cr, uid, ids, context=context):
            if voucher.journal_id.type in ['purchase','purchase_refund'] and voucher.pay_journal_id.type == 'cash'\
               and voucher.pay_type != 'cash':
                return False
        return True

    def _check_voucher_date(self, cr, uid, ids, context=None):
        for voucher in self.browse(cr, uid, ids, context=context):
            if voucher.date > str(datetime.date.today()):
                return False
        return True
    ...
    _constraints = [(_check_permission_uniq, 'Permission must be unique!', ['permission']),
                    ...
                    (_check_pay_type, 'Pay type must be cash, because payment method is cash', ['Pay Type']),
		    (_check_voucher_date, 'You Cann\'t  create Account Voucher in future date!', ['Date']),
                    ]
    ...
    def copy_dr_line(self, cr, uid, ids, context=None):
        ...
        """
        voucher_line_obj = self.pool.get('account.voucher.line')
        default = {'amount':0.0, 'custody':False,'custody_end_date':False, 'res_partner_id':False,'untax_amount':0.0}
    ...
    def cancel_voucher(self, cr, uid, ids, context={}):
        ...
        for voucher in self.browse(cr,uid,ids,context):
	    if voucher.move_id:
                if voucher.move_id.state == 'posted':
                     raise osv.except_osv(_('Error!'), _('You can\'t cancel voucher with Posted Journal'))
...
# Check Range Class
#----------------------------------------------------------
class check_range(osv.Model):

    """ Used to Add range to Bank Check Sequence """

    _name = 'check.range'
    _name_req = 'journal_id'
    _columns = {
        'journal_id' : fields.many2one('account.journal','Journal'),
        'first_number': fields.integer('First Number'),
        'last_number': fields.integer('Last Number'),
        'checks_number': fields.integer('Number of checks'),
        'archive': fields.boolean('Archive'),
               }
    ...
    _sql_constraints = [
        ('first_number_positive', 'check(first_number > 0)', 'The First Number field must be greater than 0.'), 
        ('last_number_positive', 'check(last_number > 0)', 'The Last Number field must be greater than 0.'),    
        ('checks_number_positive', 'check(checks_number > 0)', 'The Number of checks field must be greater than 0.'),   
        ('first_last_number_check', 'check(first_number < last_number)', 'The Last Number must be greater than first Number.'),    
            ]

    def onchange_range(self, cr, uid, ids, first_number, last_number):
        """ 
        ...
        """
        checks_number = last_number - first_number + 1
        result={'checks_number': checks_number}
        res = {'value': result}
        return res

    def onchange_checks_number(self, cr, uid, ids, first_number, checks_number):
        """ 
        change last_number field when changing checks_number fields.
        @param checks_number: number of checks
        @return: Dictionary of values of last_number 
        """
        last_number  = checks_number + first_number - 1
