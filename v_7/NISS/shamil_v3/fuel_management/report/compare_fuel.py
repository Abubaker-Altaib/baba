...
class compare_fuel(report_sxw.rml_parse):
    ...
    def _getdata(self, data):
        ...
        lines_ids = fuel_quantity.search(self.cr, self.uid, ['&', '|', '&', ('month', '=', month), (
            'year', '=', year), ('month', '=', month1), ('year', '=', year1)], context=self.context)
        for quantity in fuel_quantity.browse(self.cr, self.uid, lines_ids, context=self.context):
            if bool(place_id):
                if int(quantity.plan_id.place_id.id) != int(place_id[0]):
                    continue
            department = quantity.department_id and quantity.department_id.name or ''
            plan_type = quantity.plan_type
            q = lines.get(department + '|' + plan_type,
                          {'department': department, 'plan_type': plan_type, 'month1': 0, 'month2': 0})
...
report_sxw.report_sxw('report.compare_fuel.report', 'fuel.plan',
                      'addons/fuel_management/report/compare_fuel.rml', parser=compare_fuel, header=False)
