...
class fuel_plan_report(report_sxw.rml_parse):
    ...
    def _getdata(self, data):
        ...
        place_id = data['form']['place_id']
        department_obj = self.pool.get('hr.department')

        if department:
            department_id = department[0]
            department_ids = [department_id]
            department_ids.append(department_obj.search(
                self.cr, self.uid, [('id', 'child_of', department_id)]))
        ...
        for plan in fuel_plan.browse(self.cr, self.uid, account, context=self.context):
            if place_id:
                    if plan.place_id.id != place_id[0]:continue
            ...
            for quantity in fuel_quantity.browse(self.cr, self.uid, plan.quantity_ids, context=self.context):
                ...
                lines_ids = fuel_qty_line.search(
                    self.cr, self.uid, [('qty_id', '=', quantity.id.id)])
    ...
    def _getdata3(self, data):
        ...
        hr_department = self.pool.get('hr.department')
        h = hr_department.browse(
            self.cr, self.uid, [department_id], context=self.context)
        res = {}
...
report_sxw.report_sxw('report.fix_fule_plan.report', 'fuel.plan',
                      'addons/fuel_management/report/fuel_plan_report.rml', parser=fuel_plan_report, header=False)
