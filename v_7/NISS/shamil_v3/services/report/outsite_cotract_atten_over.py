...
WHERE  
  out.id = mount.employee and mount.overtime_name=3 AND out.id=%s and mount.month=%s and mount.year='%s' ''',(l['empid'],month,year))
            emp_attent = self.cr.dictfetchall()
            if len(emp_attent)>0:
               apsent_day= emp_attent[0]['no_day']
               attent_day=int(30 if emp_attent[0]['no_day'] is 0 else 30-apsent_day)
            no+=1
