...
class report_12c_recive(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(report_12c_recive, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        for obj in self.pool.get('stock.picking.out').browse(self.cr, self.uid, ids, self.context):
            if obj.state != 'done':             
               raise osv.except_osv(_('Error!'), _('You cannot open this report now , please recived this order firstly'))
        return super(report_12c_recive, self).set_context(objects, data, ids, report_type=report_type)
    ...
    def _getShop(self, ids):
        p = self.pool.get('stock.picking.out')
...
report_sxw.report_sxw('report.report_12c_recive', 'stock.picking.out', 'addons/stock_niss/report/report_12c_recive.rml' ,parser=report_12c_recive,header=False)
