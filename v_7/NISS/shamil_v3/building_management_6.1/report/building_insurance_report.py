...
class building_insurance_report(report_sxw.rml_parse):
    ...
    def _getdata1(self,data):
        ...
        if building_id:
            ...
            insurance_lines = insurance_line_obj.browse(self.cr, self.uid,insurance_line_ids)
            insurance_ids = [line.insurance_id.id for line in insurance_lines]
            insurance_ids = list(set(insurance_ids))
            domain.append(('id','in',insurance_ids))
        if state :
            if state =='completed':
