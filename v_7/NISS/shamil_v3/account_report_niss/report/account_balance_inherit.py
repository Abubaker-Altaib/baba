...
class account_balance(report_sxw.rml_parse, common_header, custom_common_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if (data.get('model', {}) == 'ir.ui.menu'):
            ...
            new_ids = data['form']['account_id'] and [data['form']['account_id'][0]] or 'account_ids' in data['form'] and data['form']['account_ids'] or 'chart_account_id' in data['form'] and [data['form']['chart_account_id']] or [] 
            if data['form']['currency_id']:
                new_ids = self.pool.get('account.account').search(self.cr, self.uid, [('currency_id','=',data['form']['currency_id'][0] ), ('type','!=','view'), ('id', 'child_of', new_ids)], order='code')
...
report_sxw.report_sxw('report.account.balance.arabic.inherit', 'account.account', 'addons/account_report_niss/report/account_balance.rml', parser=account_balance , header='external')
