    ...
    def get_data(self,data):
           ...
           self.cr.execute( """
                                    select                        
                                                    ...
                                                    left join product_product pd on (ir_prod.product_id=pd.id)
                                                    left join product_template pdc on (pdc.id=pd.product_tmpl_id)
           ...
           self.cr.execute( """  
                               select string_agg(dept.name , ' + ' ) as department , string_agg(ir.name , ' - ' ) as orders, string_agg(ir.purchase_purposes , ' + ' ) as purposes
                                      from ireq_m ir
