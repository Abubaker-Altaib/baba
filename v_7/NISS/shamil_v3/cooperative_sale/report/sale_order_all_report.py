...
# Method to get data      
      def get_sale_order_data(self,data):
           ...
           conditions = " and so.company_id=(%s)"%company_id[0] 
	   if payroll_id :
           		conditions = conditions + "and emp.payroll_id=%s"%payroll_id[0]
           if category_id :
	   ...
	   if report_type == 'deduction' : 
		...
		if  payment_type == 'cash' and order_type == 'specific' : 
			conditions = conditions + " and so.payment_type='%s'"%payment_type
           		self.cr.execute( """
                		select                        
                                	distinct so.name as name ,
		...
		if  payment_type == 'cash' and order_type != 'specific' : 
			conditions = conditions + " and so.payment_type='%s'"%payment_type
           		self.cr.execute( """
					...
					left join sale_order_line l on (so.id = l.order_id)
                            where so.order_type !='specific' and (to_char(so.create_date,'YYYY-mm-dd')>=%s and to_char(so.create_date,'YYYY-mm-dd')<=%s) and so.state in ('done') and (so.print_order = True or so.barcode_order = True)
                """ + conditions + """group by so.id,so.name,emp.otherid,so.date_order,partner.name,partner.code,partner.street,partner.phone,so.state order by so.name  """  ,(date1,date2,))  
           		move = self.cr.dictfetchall()
      ...
      def _gettotal(self,data):
	   ...
	   if report_type == 'deduction' : 
		...
		if  payment_type == 'cash' :  
              		conditions = conditions + " and so.payment_type='%s'"%payment_type
              		conditions = conditions + " and so.order_type='%s'"%order_type
           		self.cr.execute( """
