    ...
    def get_sale_order_data(self,data):
           ...
           for order_id in order_lines :
                ...
                for line in move :
                    ...
                    ######################### Process From stock after print @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
                    check_picking = picking_obj.search(self.cr ,self.uid,[('sale_id','=',[line['id']]),('state','not in',('done','cancel'))],context=self.context)
                    picking_obj.write(self.cr ,self.uid , check_picking , {'state' : 'done'},context = self.context)
                    move_ids = stock_move_obj.search(self.cr ,self.uid,[('picking_id','in',check_picking)],context=self.context)
                    stock_move_obj.write(self.cr ,self.uid, move_ids , {'state' : 'done'},context = self.context) 
                    ######################### Process From stock after print @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
