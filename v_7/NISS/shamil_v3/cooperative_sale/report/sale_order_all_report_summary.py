...
class sale_order_all_report_summary(report_sxw.rml_parse):
      def __init__(self, cr, uid, name, context):
        super(sale_order_all_report_summary, self).__init__(cr, uid, name, context)
...
# Method to get data      
      def get_sale_order_data(self,data):
	   ...
	   print_order = data['form']['print_order']
	   start_order = data['form']['start_order']
	   ...
	   if print_order :
			conditions = conditions + " and so.print_order='True'"
	   if date1 and date2 :
	   	if start_order :
			conditions = conditions + " and (to_char(so.start_date,'YYYY-mm-dd')>='%s'"%date1
			conditions = conditions + " and to_char(so.start_date,'YYYY-mm-dd')<='%s')"%date2
	   	if not start_order :
			conditions = conditions + " and (to_char(so.date_order,'YYYY-mm-dd')>='%s'"%date1
			conditions = conditions + " and to_char(so.date_order,'YYYY-mm-dd')<='%s')"%date2
	   if report_type == 'deduction' : 
           		...
           		self.cr.execute( """
					...
					so.state as state ,
                                ...
                                From sale_order so
                                	...
                                	left join product_category cat on (so.product_cat=cat.id)
					left join sale_order_line l on (so.id = l.order_id)
                            where so.payment_type in ('installment','up_cash') and so.state in ('done') 
                """ + conditions + """group by l.period,so.id,so.name,emp.otherid,so.date_order,emp.name_related,emp.degree_id,emp.emp_code,so.state order by so.name  """) 
		...
		if  payment_type == 'cash' : 
			conditions = conditions + " and so.payment_type='%s'"%payment_type
                ...
                """ + conditions + """group by so.id,so.name,emp.otherid,so.date_order,emp.name_related,emp.degree_id,emp.emp_code,so.state order by so.name  """) 
           		move = self.cr.dictfetchall()
	   	...
	   	if receive_state :
           	...
           	self.cr.execute(
				...
				left join stock_picking sp on (so.id = sp.sale_id)
                            ...
                            where so.deliver_order=False and (to_char(so.date_order,'YYYY-mm-dd')>=%s and to_char(so.date_order,'YYYY-mm-dd')<=%s) and so.state in ('done') 
...
report_sxw.report_sxw('report.sale_order_all_report_summary', 'sale.order', 'addons/cooperative_sale/report/sale_order_all_summary.rml' ,parser=sale_order_all_report_summary,header=False)
