...
class additional_report_bank(report_sxw.rml_parse):
    ...
    def _getShop(self,data,bank_id):
        ...
        self.cr.execute("SELECT hr_employee.id,rpb.acc_number AS employee_code, "\
                    ...
                    "COALESCE(hr_employee_mission_line.gross_amount,0) AS total "\
                    "FROM hr_employee_mission_line "\
                    "left join hr_employee_mission a on a.id = hr_employee_mission_line.emp_mission_id "\
                    "left join hr_employee ON (hr_employee_mission_line.employee_id = hr_employee.id) "\
...
report_sxw.report_sxw('report.mission_bank', 'hr.employee.mission', 'addons/hr_ntc_custom/report/mission_report_bank.rml' ,parser=additional_report_bank,header=False)
