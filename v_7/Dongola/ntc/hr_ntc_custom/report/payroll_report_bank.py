...
class payroll_report_bank(report_sxw.rml_parse):
    ...
    def _getShop(self,data,bank_id):
        ...
        self.cr.execute("select m.employee_id from hr_payroll_main_archive as m "\
                        "where m.bank_account_id in %s and month= %s and year = %s and in_salary_sheet = %s" 
                        , (tuple(bank_id_list),data['form']['month'],data['form']['year'], salary_sheet))
        ...
        #-------------------------------------------------------------------------------
            ...
            if salary_sheet:
                self.cr.execute("SELECT hr_employee.id,rpb.acc_number AS employee_code, "\
                            ...
                            "and hr_payroll_main_archive.bank_account_id in %s " \
                            ...
                            (data['form']['month'],data['form']['year'],tuple(emp_ids),tuple(bank_id_list),
...
report_sxw.report_sxw('report.payroll.report.bank', 'hr.payroll.main.archive', 'addons/hr_ntc_custom/report/payroll_report_bank.rml' ,parser=payroll_report_bank,header=False)
