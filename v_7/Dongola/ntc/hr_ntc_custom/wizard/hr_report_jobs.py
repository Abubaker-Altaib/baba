...
class employee_by_department(osv.osv_memory):
    ...
    _inherit= "hr.report.jobs"

    _columns = {

        'report_type':fields.selection([
                             ('jobs', 'Jobs by Job Positions'),
                             ('department', 'Jobs by departments')] , "Report Type", required=True),
    }
    '''_columns = {
    ...
    def print_report(self, cr, uid, ids, context={}):
        ...
        if data['form']['report_type'] == 'jobs':
            return {'type': 'ir.actions.report.xml', 'report_name': 'hr.employee.job', 'datas': data}
        else:
            return {'type': 'ir.actions.report.xml', 'report_name': 'hr.department.job', 'datas': data}
