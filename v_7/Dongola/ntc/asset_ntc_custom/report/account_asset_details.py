...
class assets_details(report_sxw.rml_parse):
    ...
    def compute_assets(self, company_id, category_ids, date):
        ...
        for asset in asset_obj.browse(self.cr, self.uid, search_ids):
            ...
            self.assets[asset.category_id.id][asset.name]['duration'] = asset.category_id.depreciation_rate * 100
        ...
        for key in self.assets.keys():
            ...
            'value':0,  'sum_value':0, 'depreciation':0, 'rest_value':0, 'duration':0}
            for record in dic:
                ...
                last_line['duration'] += self.assets[key][record]['duration']
            ...
            print last_line,"last_linelast_linelast_linelast_line"
            #to round the percent to 2 digits
            #if last_line['depreciation'] and last_line['sum_value']:
            self.depreciation_percent[counter] = last_line['duration']
...
report_sxw.report_sxw('report.account.assets.details.ntc.report',
                      'account.asset.asset',
