...
class asset_list(report_sxw.rml_parse, common_report_header):
    ...
    def lines_depre(self):
        ...
        self.cr.execute("select max(depreciation_date) as last_depr_date,a.name as name,a.purchase_date as purchase_date,\
                       ...
                       WHERE a.state='open' and depr.move_check=True and move_check=True "+self.where_clause+" \
                       GROUP BY a.name,a.code,a.purchase_date,a.purchase_value,a.value_residual,a.purchase_value,\
                                    cat.name,a.method_time,a.method_end,a.depreciation_rate\
                           ")
    ...
    def lines(self):
        ...
        self.cr.execute("SELECT  a.name, a.code, h.date as history_date, a.purchase_date,  min(cat.name) as cat_name,\
                        a.purchase_value,h.asset_value,SUM(COALESCE(h.amount,0)) as amount,  SUM(COALESCE(current_value,0)) as current_value\
                        from account_asset_history h \
                        LEFT JOIN account_asset_asset a on (h.asset_id=a.id)\
                        LEFT JOIN account_asset_category cat on (cat.id= a.category_id) \
                        where  h.state= 'posted' " +self.where_clause+ "\
                        group by   a.name, a.code, a.purchase_value, h.asset_value,h.date, a.purchase_date ") 
        res = self.cr.dictfetchall()
        for r in res:
            self.sum_residual += r['amount'] or 0
            self.sum_purchase += r['asset_value']- r['amount'] or 0
        return res
...
report_sxw.report_sxw('report.account.asset.operation.ntc3', 'account.asset.category', 'addons/asset_ntc_custom/report/account_asset_operation.rml', parser=asset_list, header=False)

report_sxw.report_sxw('report.account.asset.Depr.ntc', 'account.asset.category', 'addons/account_asset_custom/report/account_asset_depre.rml', parser=asset_list, header=False)
