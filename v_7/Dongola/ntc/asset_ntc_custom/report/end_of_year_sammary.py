    ...
    def compute_assets(self, company_id, category_ids, date):
        ...
        for cat in cat_obj.browse(self.cr, self.uid, search_caat):
            ...
            for asset in asset_obj.browse(self.cr, self.uid, search_ids):
                ...
                self.assets[cat.id]['value_during_year'] += value_during_year
                ...
                self.assets[cat.id]['depreciation_during_year'] += math.fabs(depreciation_during_year)
                ...
                depreciation_compound = sum(x.depreciated_value for x in asset.depreciation_line_ids\
                 if x.depreciation_date and\
                    (self.to_date(x.depreciation_date) <= start_date))
                self.assets[cat.id]['depreciation_compound_first'] += math.fabs(depreciation_compound)

                #get the cut_depreciation_sum_year for the asset 
                cut_depreciation_sum_year = sum(x.amount for x in asset.history_ids\
                 if x.type in ['abandon','sale'] and (self.to_date(x.date) <= start_date))

                self.assets[cat.id]['cut_depreciation_sum_year'] += math.fabs(cut_depreciation_sum_year)

                if self.assets[cat.id]['value_at_start']:
                    abandon_rate = (self.assets[cat.id]['cut_depreciation_sum_year'] / self.assets[cat.id]['value_at_start'])*100
                    
                self.assets[cat.id]['cut_depreciation_sum_year_last'] = round(self.assets[cat.id]['depreciation_compound_first'] * abandon_rate,2)
                
                self.assets[cat.id]['depreciation_compound_second'] = round(self.assets[cat.id]['depreciation_compound_first'] - self.assets[cat.id]['cut_depreciation_sum_year_last'],2)
            
            
            #get duration

            #if self.assets[cat.id]['value_at_start']:
            self.assets[cat.id]['duration'] = cat.depreciation_rate * 100
...
report_sxw.report_sxw('report.account.assets.end_of_year_ntc.report',
                      'account.asset.asset',
