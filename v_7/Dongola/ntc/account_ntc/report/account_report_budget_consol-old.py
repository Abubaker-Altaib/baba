...
class account_budget_consol(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_budget_consol, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_budget_consol, self).set_context(objects, data, ids, report_type=report_type)
    ...
    def _get_children_and_consol_analytic(self, cr, uid, ids, context=None):
        ...
        print "7777777777777777777777777",ids,ids2           
        return ids2
...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    def get_budget(self, form):
        ...
        print "??????????????????????????????????????????",company_ids,analytic_account_ids 
        return res
    ...
    def get_budget_line_view(self, form, child_ids):
        ...
        planned_amount=0
        print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.PARENTS UNIQU",parents_uniqe
        ...
        for ac in main_acc:
            ...
            while(have_parent):
                ...
                print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>WH WH WH WH",ac, wh_ac.parent_id.id
        ...
        print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>PARETTTTTTTTT ",all_parent ," |||| ",sorted(all_parent)
        ...
        for acc in self.pool.get('account.account').browse(self.cr, self.uid, all_parent, context=self.context ):
            ...
            if len(all_accounts) > 0:
                ...
                budget.append({
               'is_main':is_main, 'account_name':acc.name,            'account_code':acc.code,'account_id':acc.id,'balance':balance,'planned':planned_amount,'total_operation':total_operation,'relative':relative,'ratio':round(ratio,1)})
                print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>IS MAIN",is_main
        return budget
        ...
        if is_main:
            ...
            for acc in self.pool.get('account.account').browse(self.cr, self.uid,childs, context=self.context):
                ...
                if planned_amount>0:
                    ...
                    ratio=round((balance/relative)*100,2)
                budget.append({
                'account_name':acc.name,            'account_code':acc.code,'account_id':acc.id,'balance':balance,'planned':planned_amount,'total_operation':total_operation,'relative':relative,'ratio':round(ratio,1)})
...
report_sxw.report_sxw('report.account.budget.consol',  'account.report.budget', 'addons/account_ntc/report/account_report_budget_consol.rml', parser=account_budget_consol , header='external')
