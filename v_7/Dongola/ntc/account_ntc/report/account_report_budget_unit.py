...
class account_budget_unit(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_budget_unit, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_budget_unit, self).set_context(objects, data, ids, report_type=report_type)
...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    ...
    def _types(self,form,child_ids):
        ...
        for type in types:
            ...
            for line in self.pool.get('account.budget.lines').browse(self.cr, self.uid, lines, context=self.context):
                ...
                #out budget 
                if line.account_budget_id.company_id.out_budget==True:
                    planned_out+=line.planned_amount
                else:
                    planned_in+=line.planned_amount
           ...
           'balance':balance,'planned':planned,'planned_out':planned_out,'planned_in':planned_in,'total_operation':total_operation,'relative':relative,'ratio':ratio})
    ...
    def get_budget_line_view(self, form, child_ids,type):
        ...
        for b in self.pool.get('res.company').browse(self.cr, self.uid, company_ids, context=self.context):
            ...
            sub_company_in_ids= self.pool.get('res.company').search(self.cr, self.uid, [('parent_id', '=', b.id),('out_budget', '=',False) ], context=self.context)
            sub_company_out_ids= self.pool.get('res.company').search(self.cr, self.uid, [('parent_id', '=', b.id),('out_budget', '=',True) ], context=self.context)
            ...
            analytic_account_out_ids= self.pool.get('account.analytic.account').search(self.cr, self.uid, [('id', 'in', child_ids),('company_id', 'in', sub_company_out_ids)]) 
            analytic_account_in_ids= self.pool.get('account.analytic.account').search(self.cr, self.uid, [('id', 'in', child_ids),('company_id', 'in', sub_company_in_ids)])  
 
            if not analytic_account_ids:
               continue
            budget_id2s = budget_pool.search(self.cr, self.uid, [('analytic_account_id', 'in', analytic_account_ids)], context=self.context)
            budget_id2s_out = budget_pool.search(self.cr, self.uid, [('analytic_account_id', 'in', analytic_account_out_ids)], context=self.context)
            budget_id2s_in = budget_pool.search(self.cr, self.uid, [('analytic_account_id', 'in', analytic_account_in_ids)], context=self.context)
            total_operation=0.0
            analytic_account_ids = ','.join(map(str,  analytic_account_ids))
            #analytic_account_out_ids = ','.join(map(str,  analytic_account_out_ids))
            #analytic_account_in_ids = ','.join(map(str,  analytic_account_in_ids))

            for budget in self.pool.get('account.budget').browse(self.cr, self.uid, budget_id2s_out, context=self.context):
                for line in budget.account_budget_line:
                    planned_amount_out+=line.planned_amount
 

            for budget in self.pool.get('account.budget').browse(self.cr, self.uid, budget_id2s_in, context=self.context):
                for line in budget.account_budget_line:
                    planned_amount_in+=line.planned_amount
            ...
            if  relative==0:
                ...
                ratio=round((balance/relative)*100,2) 
            companies.append({
            'name':b.name, 'planned':planned_amount,'planned_out':planned_amount_out,'planned_in':planned_amount_in,'relative':relative,'balance':balance,'ratio':ratio, })
        return companies
...
report_sxw.report_sxw('report.account.budget.unit',  'account.report.budget', 'addons/account_ntc/report/account_report_budget_unit.rml', parser=account_budget_unit , header='external')
