...
class account_budget_sub(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_budget_sub, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_budget_sub, self).set_context(objects, data, ids, report_type=report_type)
...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    ...
    def _types(self,form,child_ids):
        ...
        company_name = self.pool.get('res.users').browse(self.cr, self.uid, self.uid, context=self.context).company_id.name
        ...
        budget.append({ 'name':company_name,'company_ids':company_ids,
           'balance':balance,'planned':planned,'total_operation':total_operation,'relative':relative,'ratio':ratio})
   
        return budget
   
    def get_budget_line_view(self, form, child_ids,company_ids):
        budget_pool = self.pool.get('account.budget')
        move_line_pool = self.pool.get('account.move.line')
        fiscalyear_pool = self.pool.get('account.fiscalyear')
        ...
        for b in self.pool.get('res.company').browse(self.cr, self.uid, company_ids, context=self.context):
            ...
            analytic_account_ids= self.pool.get('account.analytic.account').search(self.cr, self.uid, [('id', 'in', child_ids),('company_id', '=',b.id)]) 
...
report_sxw.report_sxw('report.account.budget.sub',  'account.report.budget', 'addons/account_ntc/report/account_report_budget_sub.rml', parser=account_budget_sub , header='external')
