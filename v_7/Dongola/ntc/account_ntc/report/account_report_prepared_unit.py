...
class account_prepared_budget_unit(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_prepared_budget_unit, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_prepared_budget_unit, self).set_context(objects, data, ids, report_type=report_type)
    ...
    def _get_children_and_consol(self, cr, uid, ids,code, context=None):
        ...
        ids2 = self.pool.get('account.account').search(cr, uid, [('parent_id', 'in', ids)], context=context)
        ids3 = []
        for rec in self.pool.get('account.account').browse(cr, uid, ids2, context=context):
            for child in rec.child_consol_ids:
                ids3.append(child.id)
...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    ...
    def _types(self,form,child_ids):
        ...
        for type in types:
            ...
            # add code to analytiv account
 

            
            lines=  self.pool.get('account.fiscalyear.budget.lines').search(self.cr, self.uid, [('account_fiscalyear_budget_id', 'in', budget_ids),('company_id', 'in', company_ids)], context=self.context)
    ...
    def get_budget_line_view(self, form, child_ids,type):
        ...
        for b in self.pool.get('res.company').browse(self.cr, self.uid, company_ids, context=self.context):
            ...
            # accounts= self.pool.get('account.account').search(self.cr, self.uid, [('code', '=', code),('company_id', '=', b.id)], context=self.context)
            ...
            for budget in self.pool.get('account.fiscalyear.budget').browse(self.cr, self.uid, budget_id2s, context=self.context):
                for line in budget.account_fiscalyear_budget_line:
...
report_sxw.report_sxw('report.account.prepared.budget.unit',  'account.report.budget', 'addons/account_ntc/report/account_report_prepared_unit.rml', parser=account_prepared_budget_unit , header='external')
