...
class account_assistant_report(report_sxw.rml_parse, common_header, custom_common_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        ctx2['unit_type'] =data.get('form', {}).get('unit_type', False)     
    ...
    def account_partners(self, form,code):
        ...
        if ctx['fiscalyear']:
            ...
            date_from = datetime.datetime.strptime(ctx['date_from'], '%Y-%m-%d')
        ...
        if date_from==date_start:
            ...
            self.cr.execute(
                "select sum(debit) as debit,sum(credit) as credit, 0 as init_bal ,p.id as p_id,p.name as name  from  account_move_line l \
                 join account_move m on m.id=l.move_id\
                 join res_partner p on p.id=l.partner_id\
                 where l.account_id in %s and l.date >%s and l.date <= %s and l.period_id in %s  \
                 group by p.id,p.name  order by p.name", (tuple(accounts), ctx['date_from'],ctx['date_to'],tuple(period_ids) ,))
            ...
            self.cr.execute(
                "select (sum(debit)-sum(credit)) as init_bal,0 as debit,0 as credit ,p.id as p_id,p.name as name  from  account_move_line l \
                 join account_move m on m.id=l.move_id\
                 join res_partner p on p.id=l.partner_id\
                 where l.account_id in %s and l.date <= %s and l.period_id in %s   \
                 group by p.id,p.name order by p.name", (tuple(accounts), ctx['date_from'],tuple(period_ids)))
            ...
            self.cr.execute(
                "select sum(debit) as debit,sum(credit) as credit, 0 as init_bal ,p.id as p_id,p.name as name  from  account_move_line l \
                 join account_move m on m.id=l.move_id\
                 join res_partner p on p.id=l.partner_id\
                 where l.account_id in %s and l.date between %s and %s and l.period_id in %s  \
                 group by p.id,p.name  order by p.name", (tuple(accounts), ctx['date_from'],ctx['date_to'],tuple(period_ids) ,))
        ...
        full_account = [r for r in res2 if r['init_bal']!= 0.00 or r['debit']!= 0.00 or r['credit']!= 0.00]
        full_account=sorted(full_account, key=lambda k:k['name'])
