...
class account_assistant_report(report_sxw.rml_parse, common_header, custom_common_header):
    ...
    def lines(self, form, code,ids=[], done=None):
        ...
        res2=[]
        acc_id=0
        account = self.pool.get('account.account').search(self.cr,self.uid, [('code', '=', code)],context=self.context)
        for acc in  self.pool.get('account.account').browse(self.cr,self.uid, account,context=self.context):
            acc_id=acc.id
        res1=filter(lambda acc: acc['parent_id'][0] ==acc_id and (acc['balance']+acc['init_balance']!=0),self.result_acc)
        for acc in  self.pool.get('account.account').browse(self.cr,self.uid, form['account_ids'],context=self.context):
            sign = acc.user_type.report_type in ('income','liability') and -1 or 1
        for item in res1:
            if item not in res2:
               res2.append(item)
        # SIGN HANDLING
        for it in res2:
            for acc in  self.pool.get('account.account').browse(self.cr,self.uid, [it['id']],context=self.context):
                sign = acc.user_type.report_type in ('income','liability') and -1 or 1
    ...
    def lines2(self, form, ids=[], done=None):
        ...
        if form['account_ids']:
            ...
            self.result_acc=filter(lambda acc: acc['code'] == code,self.result_acc)
        res2=[]
    ...
    def _get_parents(self,data):
        if data['account_ids']:
           ...
           res=filter(lambda acc: acc['code'] in codes and (acc['debit']+acc['credit']+acc['init_balance']!=0),self.result_acc)
           return res
        ...
        if self.init_query:
            self.cr.execute(
                ...
                "FROM   (SELECT p.id, l.move_id, p.name AS name, "\
                            ...
                            "WHERE l.partner_id = p.id AND am.id = l.move_id " + state_query + " AND account_id = %s AND " + self.init_query + ") AS init_bal "\
                    "FROM account_move_line l LEFT JOIN res_partner p ON (l.partner_id=p.id) JOIN account_move am ON (am.id = l.move_id)" \
                    "WHERE l.account_id = %s " + state_query + " AND " + self.query + " " + partner_query + " " \
