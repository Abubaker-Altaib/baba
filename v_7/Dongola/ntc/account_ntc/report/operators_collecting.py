...
class operators_collecting_report(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        self.context = context
        super(operators_collecting_report, self).__init__(cr, uid, name, context)
    ...
    def lines(self,data):
        ...
        sum_all = 0
        sum_all_debit=sum_all_credit=0
        for partner in self.operators:
            ...
            for key in keys:
                ...
                if debit == 0:
                    continue
                credit_percent = round(100*(credit*1.0)/(debit*1.0),2)
                rest = debit - credit
                rest_percent = round(100*(rest*1.0)/(debit*1.0),2)
                line = {
                        'name'   :self.accounts[key],
                        'debit'  :str(debit),
                        'credit' :str(credit or 0.0),
                        'credit_percent'   :str(credit_percent)+"%",
                        'rest':str(rest or 0.0),
                        'rest_percent':str(rest_percent)+"%"
                        }
                lines.append(line)
        ...
        if sum_all_debit != 0:
            debit = sum_all_debit
            credit = sum_all_credit
            credit_percent = round(100*(credit*1.0)/(debit*1.0),2)
            rest = debit - credit
            rest_percent = round(100*(rest*1.0)/(debit*1.0),2)
            line = {'name'   :'إجمالي كل الشركات',
                    'debit'  :str(debit),
                    'credit' :str(credit or 0.0),
                    'credit_percent'   :str(credit_percent)+"%",
                    'rest':str(rest or 0.0),
...
report_sxw.report_sxw('report.operator_collecting_report.report', 'operator.report', 'addons/account_ntc/report/operators_collecting.rml' ,parser=operators_collecting_report,header=False )
