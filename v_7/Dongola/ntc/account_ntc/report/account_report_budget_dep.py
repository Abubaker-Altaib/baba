...
class account_budget_dep(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_budget_dep, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_budget_dep, self).set_context(objects, data, ids, report_type=report_type)
...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    ...
    def get_budget_line_view(self, form, child_ids):
        ...
        for b in budget_pool.browse(self.cr, self.uid, budget_ids, context=self.context):
            planned_amount=0
            balance=0.0
            total_operation=0.0
            for line in b.account_budget_line:
                planned_amount+=line.planned_amount
                total_operation+=line.total_operation
            self.cr.execute("select (sum(debit)-sum(credit)) as bl\
                   from account_move_line  where analytic_account_id in (%s)  and date>= '%s' and date <= '%s'" % (b.analytic_account_id.id,date_from,date_to)) 
...
report_sxw.report_sxw('report.account.budget.dep',  'account.report.budget', 'addons/account_ntc/report/account_report_budget_dep.rml', parser=account_budget_dep , header='external')
