...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    ...
    def get_budget_line_view(self, form, budget_id):
        ...
        for acc in self.pool.get('account.account').browse(self.cr, self.uid, parents_uniqe, context=self.context ):
            ids2 = self.pool.get('account.account').search(self.cr, self.uid, [('parent_id', 'child_of', acc.id)], context=self.context)
            planned=0
            balance=0.0
            total_operation=0.0
            ...
            budget.append({
            'account_name':acc.name,            'account_code':acc.code,'account_id':acc.id,'balance':balance,'planned':planned,'total_operation':total_operation,'relative':relative,'ratio':ratio})
        ...
        for line in bud.account_budget_line:
            if line.general_account_id.parent_id.id == parent_id:
                ...
                if date_from and date_to:
                    ...
                    planned = line.planned_amount
                    sign = line.general_account_id.user_type.report_type in ('income','liability') and -1 or 1
                    mov = move_pool.search(self.cr, self.uid, [('company_id', '=', bud.company_id.id),('date','>=',date_to),('date','<',date_from)])
                    previous_move_ids = move_line_pool.search(self.cr, self.uid, [('analytic_account_id', '=', bud.analytic_account_id.id),('date','>=',fiscalyear_date_start),('date','<',date_from),('account_id', '=', line.general_account_id.id),('state','=','valid'),('move_id','in',mov)], context=self.context)
                    pre_balance = 0
                    ...
                    move = move_line_pool.search(self.cr, self.uid, [('analytic_account_id', '=', bud.analytic_account_id.id),('date','>=',date_from),('date','<=',date_to),('account_id', '=', line.general_account_id.id),('state','=','valid'),('move_id','in',mov)], context=self.context)
                    bl = 0
                    for oo in move_line_pool.browse(self.cr, self.uid, move, context=self.context):
                        bl += oo.debit - oo.credit
                    balance = sign*bl
