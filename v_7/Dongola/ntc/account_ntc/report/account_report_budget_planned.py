...
class account_budget_planned(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        super(account_budget_planned, self).__init__(cr, uid, name, context=context)
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_budget_planned, self).set_context(objects, data, ids, report_type=report_type)
...
# # # # # # #  '1' Cost Center Details  # # # # # # #
    def get_budget(self, form):
        ...
        #year=analytic_account_pool.browse(self.cr, self.uid, analytic_account_id, context=self.context).name
    ...
    def get_budget_line_view(self, form, child_ids):
        ...
        budget_pool = self.pool.get('account.fiscalyear.budget')
        ...
        self.cr.execute("select general_account_id as acc_id from account_fiscalyear_budget_lines where account_fiscalyear_budget_id in (%s)" % (budget_ids,))
        ...
        all_parent.pop(0)
        if form.get('account_id', False):
            account_id = form.get('account_id', False)[0]
            account_ids=self._get_children_and_consol(self.cr, self.uid,account_id,'')
            print "pppppppppppppppppppp",account_ids
            all_parent=filter(lambda acc: acc in account_ids,all_parent)
        ...
        for acc in self.pool.get('account.account').browse(self.cr, self.uid, all_parent, context=self.context ):
            ...
            if len(all_accounts) > 0:
                self.cr.execute("select sum(planned_amount) as planned_amount\
                          from account_fiscalyear_budget_lines where account_fiscalyear_budget_id in (%s)  and general_account_id in (%s) " % (budget_ids,all_accounts))
                ...
                budget.append({
               'is_main':is_main, 'account_name':acc.name,            'account_code':acc.code,'account_id':acc.id,'balance':balance,'planned':planned_amount})
    ...
    def get_budget_line(self, form, child_ids,parent_id,is_main):
        ...
        if is_main:
            ...
            for acc in self.pool.get('account.account').browse(self.cr, self.uid,childs, context=self.context):
                ...
                self.cr.execute("select sum(planned_amount) as planned_amount\
                          from account_fiscalyear_budget_lines where account_fiscalyear_budget_id in (%s)  and general_account_id in (%s) " % (budget_ids,consol_ids))
                amounts=self.cr.fetchall()
                ...
                rate=month_to-month_from+1
                budget.append({
                'account_name':acc.name,            'account_code':acc.code,'account_id':acc.id,'balance':balance,'planned':planned_amount})
...
report_sxw.report_sxw('report.account.budget.planned',  'account.report.budget', 'addons/account_ntc/report/account_report_budget_planned.rml', parser=account_budget_planned , header='external')
