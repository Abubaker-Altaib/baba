...
class account_balance(report_sxw.rml_parse, common_header, custom_common_header):
    ...
    def lines(self, form, ids=[], done=None):
        ...
        ctx.update(form.get('used_context',{}))
#===============================================================================
...
#===============================================================================
        ...
        accounts = obj_account.read(self.cr, self.uid, ids, ['type', 'code', 'name', 'debit', 'credit', 'balance', 'parent_id', 'level', 'child_id', 'company_id','bl_report','with_value'], ctx)
        ctx.update({'initial_bal':True})
        init_accounts = obj_account.read(self.cr, self.uid, ids, ['balance'], ctx)
...
report_sxw.report_sxw('report.account.account.balance.arabic.wafi', 'account.account', 'addons/account_arabic_reports_wafi/report/account_balance.rml', parser=account_balance, header=True)
report_sxw.report_sxw('report.account.account.just.balance.arabic.wafi', 'account.account', 'addons/account_arabic_reports/report/account_just_balance.rml', parser=account_balance, header=True)
