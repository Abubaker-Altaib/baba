...
# -------------------------------------------------------------------
# Updates an existing account chart for a Companies..
# -------------------------------------------------------------------
class wizard_update_charts_accounts(osv.osv_memory):
    ...
    _columns = {
        'state': fields.selection([('init', 'Step 1'), ('ready', 'Step 2'),('done', 'Wizard Complete')], 'Status', readonly=True),
        'company_ids':fields.many2many('res.company','company_update_chart_rel','update_chart_id','cid','Companies',required=True),
        ...
        'view': fields.boolean("Only View Accounts in Root", help="If set, the wizard will only detect view accounts for root company."),
    }
    ...
    def _map_tax_template(self, cr, uid, wizard, tax_template_mapping, company_id,tax_template, context=None):
        """
        ...
        if tax_template and not tax_template_mapping.get(tax_template.id):
            ...
            tax_ids = tax_facade.search(cr, uid, [('name', '=', tax_template.name), ('company_id', '=', company_id.id)], context=context)
            if tax_ids:
                tax_template_mapping[tax_template.id] = tax_ids[0]

    def _map_tax_code_template(self, cr, uid, wizard, tax_code_template_mapping,company_id, tax_code_template, context=None):
        """
        ...
        if tax_code_template and not tax_code_template_mapping.get(tax_code_template.id):
            ...
            tax_code_name = (tax_code_template.id == root_tax_code_id) and company_id.name or tax_code_template.name
            tax_code_ids = tax_code_facade.search(cr, uid, [('name', '=', tax_code_name),('company_id', '=', company_id.id) ])
            if tax_code_ids:
                tax_code_template_mapping[tax_code_template.id] = tax_code_ids[0]

    def _map_account_template(self, cr, uid, wizard, account_template_mapping, company_id ,account_template, context=None):
        """
        ...
        if account_template and not account_template_mapping.get(account_template.id):
            ...
            account_ids = account_facade.search(cr, uid, [('code', '=', code),('company_id', '=', company_id.id)], context=context)
            if account_ids:
                account_template_mapping[account_template.id] = account_ids[0]

    def _map_fp_template(self, cr, uid, wizard, fp_template_mapping, company_id,fp_template, context=None):
        """
        ...
        if fp_template and not fp_template_mapping.get(fp_template.id):
            ...
            fp_ids = fp_facade.search(cr, uid, [ ('name', '=', fp_template.name),('company_id', '=',company_id.id)], context=context)
            if fp_ids:
    ...
    def _find_tax_codes(self, cr, uid, wizard, context=None):
        ...
        for company_id in wizard.company_ids:
            ...
            for tax_code_template in tax_code_templ_facade.browse(cr, uid, children_tax_code_template):
                ...
                self._map_tax_code_template(cr, uid, wizard, tax_code_template_mapping, company_id,tax_code_template, context)
    ...
    def _find_taxes(self, cr, uid, wizard, context=None):
        ...
        for company_id in wizard.company_ids:
            ...
            for tax_template in wizard.chart_template_id.tax_template_ids:
                ...
                self._map_tax_template(cr, uid, wizard, tax_template_mapping,company_id, tax_template, context)
    ...
    def _find_accounts(self, cr, uid, wizard, context=None):
        ...
        for company_id in wizard.company_ids:
            account_template_mapping = {}
            if not company_id.parent_id and wizard.view:
                children_acc_template = account_template_facade.search(cr, uid, [('parent_id', 'child_of', [root_account_id]),
                                                                                 ('type','in',['view','consolidation'])], context=context)
                children_acc_template.sort()
            for account_template in account_template_facade.browse(cr, uid, children_acc_template, context=context):
                ...
                self._map_account_template(cr, uid, wizard, account_template_mapping,company_id, account_template, context)
                ...
                # Check the account for changes.
                    ...
                    ids =company_obj.search(cr, uid, [], context=context)
                    res = company_obj.read(cr, uid, ids, ['name'], context)
                    if  account.name != account_template.name and (account.name !=company_id.name or root_account_id!= account_template.id) :
                        notes += _("The name is different.\n")
                        modified = True
                    if account.type != account_template.type and (not account_template.consolidate or company_id.parent_id) :
    ...
    def _find_fiscal_positions(self, cr, uid, wizard, context=None):
        ...
        for company_id in wizard.company_ids: 
            ...
            for fp_template in fp_template_facade.browse(cr, uid, fp_template_ids, context=context):
                ...
                self._map_fp_template(cr, uid, wizard, fp_template_mapping,company_id, fp_template, context)
    ...
    def _update_tax_codes(self, cr, uid, wizard, log, context=None):
        ...
        for wiz_tax_code in wizard.tax_code_ids:
            ...
            tax_code_name = (root_tax_code_id == tax_code_template.id) and wiz_tax_code.company_id.name or tax_code_template.name
            # Ensure the parent tax code template is on the map.
            self._map_tax_code_template(cr, uid, wizard, tax_code_template_mapping,wiz_tax_code.company_id, tax_code_template.parent_id, context)
            vals = {
                'name': tax_code_name,
                'code': tax_code_template.code,
                'info': tax_code_template.info,
                'parent_id': tax_code_template.parent_id and tax_code_template_mapping.get(tax_code_template.parent_id.id),
                'company_id': wiz_tax_code.company_id.id,
    ...
    def _update_taxes(self, cr, uid, wizard, log, tax_code_template_mapping, context=None):
        ...
        for wiz_tax in wizard.tax_ids:
            ...
            self._map_tax_template(cr, uid, wizard, tax_template_mapping,wiz_tax.company_id,tax_template.parent_id, context)
            ...
            for tax_code_template in [tmpl for tmpl in tax_code_templates_to_find if tmpl]:
                self._map_tax_code_template(cr, uid, wizard, tax_code_template_mapping,wiz_tax.company_id, tax_code_template)
            vals_tax = {
                ...
                'company_id': wiz_tax.company_id.id,
    ...
    def _update_children_accounts_parent(self, cr, uid, wizard, log, parent_account_id, context=None):
        ...
        children_ids = account_facade.search(cr, uid, [
            ...
            ('id', '!=', parent_account.id),], context=context)
        if children_ids:
            try:
    ...
    def _update_accounts(self, cr, uid, wizard, log, tax_template_mapping, context=None):
        ...
        for wiz_account in wizard.account_ids:
            ...
            self._map_account_template(cr, uid, wizard, account_template_mapping,wiz_account.company_id, account_template.parent_id, context)
            # Ensure the related tax templates are on the map.
            for tax_template in account_template.tax_ids:
                self._map_tax_template(cr, uid, wizard, tax_template_mapping,wiz_account.company_id, tax_template, context)
            # Get the tax ids
            ...
            # Calculate the account code (we need to add zeros to non-view account codes)
            code = account_template.code or ''
            if account_template.type != 'view':
                if len(code) > 0 and len(code) <= wizard.code_digits:
                    code = '%s%s' % (code, '0' * (wizard.code_digits - len(code)))
            vals = {
                'name': (root_account_id == account_template.id) and wiz_account.company_id.name or account_template.name,
                #'sign': account_template.sign,
                'currency_id': account_template.currency_id and account_template.currency_id.id or False,
                'code': code,
                'type': account_template.consolidate and wiz_account.company_id.parent_id and account_template.type or 'consolidation',
                ...
                'tax_ids': [(6, 0, tax_ids)],
                'company_id': wiz_account.company_id.id,
            }
            account_id = None
            modified = False
            ...
            if vals.get('type') == 'consolidation':
                consol_account.append({'account_id':account_id, 'code':vals.get('code'), 'company_id':vals.get('company_id')})
            ...
            if modified:
                ...
                # Set this account as the parent of the accounts that seem to be its children (brothers starting with the same code).
        ...
        # Reenable the parent_store computing on account_account and force the recomputation.
        self.pool._init = False
        self.pool.get('account.account')._parent_store_compute(cr)
        for acc in consol_account:
            consol_ids = account_facade.search(cr, uid, [('code','=',acc.get('code')),('company_id.parent_id','=',acc.get('company_id'))])
            account_facade.write(cr, uid, [acc.get('account_id')], {'child_consol_ids':[(6, 0, consol_ids)]})
        return {
            'new': new_accounts,
    ...
    def _update_fiscal_positions(self, cr, uid, wizard, log, tax_template_mapping, account_template_mapping, context=None):
        ...
        for wiz_fp in wizard.fiscal_position_ids:
            ...
            if wiz_fp.type == 'new':
                ...
                vals_fp = {
                    'company_id': wiz_fp.company_id.id,
                ...
                # Update the given fiscal position (remove the tax and account mappings, that will be regenerated later)
            ...
            if modified:
                ...
                for fp_tax in fp_template.tax_ids:
                    # Ensure the related tax templates are on the map.
                    self._map_tax_template(cr, uid, wizard, tax_template_mapping,wiz_fp.company_id, fp_tax.tax_src_id, context)
                    if fp_tax.tax_dest_id:
                        self._map_tax_template(cr, uid, wizard, tax_template_mapping,wiz_fp.company_id, fp_tax.tax_dest_id, context)
                ...
                for fp_account in fp_template.account_ids:
                    # Ensure the related account templates are on the map.
                    self._map_account_template(cr, uid, wizard, account_template_mapping,wiz_fp.company_id, fp_account.account_src_id, context)
                    if fp_account.account_dest_id:
                        self._map_account_template(cr, uid, wizard, account_template_mapping,wiz_fp.company_id, fp_account.account_dest_id, context)
    ...
    def action_update_records(self, cr, uid, ids, context=None):
        ...
        if log.has_errors() and not wizard.continue_on_errors:
            raise orm.except_orm(_('Error'), _("One or more errors detected!\n\n%s") % log.get_errors_str())
...
class wizard_update_charts_accounts_tax_code(osv.osv_memory):
    ...
    _columns = {
        ...
        'type': fields.selection([('new', 'New template'),('updated', 'Updated template'),], 'Type'),
...
class account_account_template(osv.Model):
    ...
    _columns = {
        'consolidate': fields.boolean('Consolidation in Root', help="Check this option if you want this account to be consolidation in root company."),
