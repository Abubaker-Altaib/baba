...
class allow_deduct_exception(osv.osv_memory):
    ...
    def duplicate_rec(self, cr, uid, ids, context=None): 
        ...
        for record in self.browse(cr, uid,ids):
	     ...
	     check_salary = process_obj.search(cr,uid,[('employee_id','=',employee),('action','=',record.action),('types','=',record.types),('start_date','=',record.start_date)])
             if check_salary:
              return False
        return True

    _constraints = [
        (check_dates, 'Error! Exception start-date must be lower then Exception end-date.', ['start_date', 'end_date']),(duplicate_rec, 'Error! Duplication of The Same Exception Record Not Allowed.', ['employee_id', 'action','types','start_date'])
    ]
