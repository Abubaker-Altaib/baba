...
class account_compare_budget(report_sxw.rml_parse, common_report_header):
    ...
    def _get_company_detail(self, form, analytic_acc=False):
        ...
        classification_ids = form.get('classification_ids', False)
        num_year = form.get('num_year', False)

        
        fiscalyear_ids = fiscalyear_obj.search(self.cr, self.uid, [('id', '!=', self.fiscalyear.id),('date_stop', '<', self.fiscalyear.date_start)], limit=num_year, context=self.context)
        next_fiscalyear_ids = []
        year_amount = {}
        ...
        for account in account_pool.browse(self.cr, self.uid, general_account, context=self.context):
            for year in fiscalyear_obj.browse(self.cr, self.uid, fiscalyear_ids, context=self.context):
                year_amount[year.name] = 0.0
            ...
            next_lines = fisc_budget_line_pool.search(self.cr, self.uid, [('fiscalyear_id', 'in', fiscalyear_ids),
                                 ('analytic_account_id', 'in', analytic_child_ids),
                                 ('general_account_id', '=', account.id)], context=self.context)
            ...
            if next_lines:
                for line in fisc_budget_line_pool.browse(self.cr, self.uid, next_lines, context=self.context):
                    ...
                    year_amount[line.fiscalyear_id.name] = line.planned_amount
