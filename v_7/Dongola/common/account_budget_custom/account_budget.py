...
class account_budget(osv.Model):
    ...
    _columns = {
        ...
        'account_budget_line': fields.one2many('account.budget.lines', 'account_budget_id', 'Budget Lines',
                                               readonly=True, states={'draft':[('readonly', False)]}),
        'account_cash_budget_line': fields.one2many('account.budget.lines', 'account_budget_id', 'Cash Budget Lines',
                                               readonly=True, states={'draft':[('readonly', False)]}),
                ...
                for line in budget.account_budget_line:
                    ...
                    if line.cash_residual_balance > 0:
                        account_budget_lines.transfer(cr, uid, {'type':'close_transfer', 'budget_type':'cash', 'to':to,
                                                                'line_ids':[{'line_id':line, 'amount':line.cash_residual_balance}]}, context=ctx)
...
class account_budget_lines(osv.Model):
    ...
    def transfer(self, cr, uid, vals={}, context=None):
        ...
        if not budget_line_id:
            budget_line_id = budget_line_pool.create(cr, uid, {'account_budget_id':budget_id, 'analytic_account_id': analytic_account, 'planned_amount': 0,
    ...
    def _balance_amount(self, cr, uid, ids, field_name, arg=None, context={}):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            ctx = context.copy()
            ctx.update({'move_line_ids': [move.id for move in line.move_line_ids]})
            result[line.id] = sign * (ctx.get('move_line_ids') and account_pool.read(cr, uid, line.general_account_id.id, ['balance'], context=ctx)['balance'] or 0.0)
            if not ctx.get('budget_close',False):
                if line.cash_total_operation > line.planned_amount + line.total_operation:
                    raise orm.except_orm(_('Warning!'), _("Cash budget can't be more than planned budget!"))
        return result
    ...
    def fnct_cash_residual_search(self, cr, uid, obj, name, domain=None, context=None):
        """
        This method make cash_residual_balance field searchable 
    ...
    _columns = {
        ...
        'classification': fields.related('general_account_id','budget_classification'),
        ...
        'balance': fields.function(_balance_amount, type='float',  digits_compute=dp.get_precision('Account'), string='Actual Amount',
            store={
        ...
        'state': fields.related('account_budget_id', 'state', string='State', type='char',size=64, readonly=True),
    }
    ...
    def _residual_check(self, cr, uid, ids, context=None):
        for budget in self.browse(cr, uid, ids, context=context):
            ...
            if classification and not classification[0].allow_budget_overdraw:
                if self.pool.get('res.users').has_group(cr, uid, 'account_budget_custom.group_cash_budget') and budget.cash_total_operation - budget.balance < 0:
                    raise orm.except_orm(_('Conditional Error !'), _("Cash budget can't go overdrow!"))
                if budget.planned_amount + budget.total_operation - budget.balance < 0:
                    raise orm.except_orm(_('Conditional Error !'), _("Planned budget can't go overdrow!"))
        return True
        
    #FIXME: budget_check when close budget
    _sql_constraints = [('analytic_account_period_uniq', 'unique (period_id,analytic_account_id,general_account_id)', _("You can't duplicate the account for the same budget!")),
...
class account_move(osv.Model):
    ...
    def check_analytic(self, cr, uid, ids, context=None):
        """
        Workflow function change record state to 'review_analytic'.
        
        @return: write method of object
        """
        return self.write(cr, uid, ids, {'state': 'review_analytic'}, context=context)

    def analytic(self, cr, uid, ids, context=None):
        """
        Workflow function change record state to 'analytic'.
        
        @return: write method of object
        """
        self.test_analytic(cr, uid, ids, context=context)
        return self.write(cr, uid, ids, {'state': 'analytic'}, context=context)
...
class account_fiscalyear_budget(osv.Model):
    ...
    _defaults = {
        ...
        'account_fiscalyear_budget_line': lambda self, cr, uid, context:
                [{'general_account_id': acc, 'planned_amount': 0.0} for acc in self.pool.get('account.account').search(cr, uid, [('budget_classification', '!=', False)], context=context)],
    }
    ...
    def validate(self, cr, uid, ids, context=None):
        """
        Workflow method that call cash flow wizard to distribute fiscalyear budget through selected periods
        
        @return: dictionary of action details
        """
        dummy, view_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'account_budget_custom', 'account_budget_flow_view')
        FY = self.browse(cr, uid, ids[0], context=context)
        return {
            'view_mode': 'form',
            'view_id': view_id,
            'view_type': 'form',
            'res_model': 'account.budget.cash.flow.wizard',
            'type': 'ir.actions.act_window',
            'target': 'new',
            'domain': '[]',
            'context': {
                    'default_company_id': FY.company_id.id,
                    'default_fiscalyear_id': FY.fiscalyear_id.id,
                    'default_chart_analytic_account_id': FY.analytic_account_id.id,
                    'default_analytic_account_ids':  [FY.analytic_account_id.id],
                    'default_account_ids': [l.general_account_id.id for l in FY.account_fiscalyear_budget_line],
                }
        }

    def budget_flow(self, cr, uid, ids, period_ids=False, account_ids=False, context=None):
        """
        ...
        for fiscalyear_budget in self.browse(cr, uid, ids, context=context):
            ...
            for period_id in period_ids:
                ...
                if context.get('active_model') == 'account.fiscalyear.budget':
                    wf_service.trg_validate(uid, 'account.fiscalyear.budget', fiscalyear_budget.id, 'validate', cr)
        return True
...
class budget_classification(osv.Model):
    ...
    _columns = {
        ...
        'allow_budget_overdraw': fields.boolean('Allow Budget Overdraw')
    }

    _sql_constraints = [
        ('classification_code_uniq', 'unique (company_id,code)', _("The classification code must be unique per company!"))
    ]
