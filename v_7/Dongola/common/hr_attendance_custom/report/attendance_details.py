...
class attendance_details(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        super(attendance_details, self).__init__(cr, uid, name, context)
    ...
    def get_attendance(self, start_date, end_date):
        ...
        names = hr_employee.read(self.cr, self.uid, self.employees_ids,['name_related'],context=self.context)
            ...
            if sign_in and sign_out:
                ...
                if calendar:
                    ...
                    last_hour = max(calendar, key=lambda x: x[
                                    'attendance'].hour_to)
                    over_out = float(str(
                        sign_out_time.hour) + '.' + str(100*int(str(sign_out_time.minute))/60) ) - last_hour['attendance'].hour_to

                    first_hour = min(calendar, key=lambda x: x[
                                     'attendance'].hour_from)
                    over_in = float(str(
                        sign_in_time.hour) + '.' + str(100*int(str(sign_in_time.minute))/60) ) - first_hour['attendance'].hour_from
                    if over_out > 0:
                        over_time = over_out
...
report_sxw.report_sxw('report.attendance_details.report', 'hr.attendance.percentage',
                      'addons/hr_attendance_custom/report/attendance_details.mako', parser=attendance_details)
