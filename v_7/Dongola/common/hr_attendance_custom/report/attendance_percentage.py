...
class attendance_percentage(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        super(attendance_percentage, self).__init__(cr, uid, name, context)
    ...
    def lines(self,data):
        ...
        with_problems = data['form']['w_prob']
        ...
        if department_ids:
            ...
            for dep_id in department_ids:
                ...
                new_deps_ids = self.pool.get('hr.department').search(self.cr, self.uid, [('id','child_of',dep_id)], context=self.context)
        ...
        if not new_emp_ids:
            self.cr.execute(
                '''SELECT emp.id  FROM hr_employee emp 
                    ...
                    WHERE emp.id in '''+str(tuple(self.employees_ids))+'''
                    ORDER BY  deg.sequence,emp.name_related''' )  
            new_emp_ids = self.cr.dictfetchall()
            self.employees_ids = [l['id'] for l in new_emp_ids]
        ...
        for emp in employees_obj.browse(self.cr,self.uid,self.employees_ids,self.context):
            ...
            if emp_list['period_work_hours'] != 0:
                if with_problems and emp_list['percent'] != 100.0:
...
report_sxw.report_sxw('report.attendance_percentage.report', 'hr.attendance.percentage', 'addons/hr_attendance_custom/report/attendance_percentage.mako' ,parser=attendance_percentage)