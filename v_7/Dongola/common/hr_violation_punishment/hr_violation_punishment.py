...
class hr_punishment(osv.Model):
    ...
    def copy(self, cr, uid, ids, default={}, context=None):
        """
        This method duplicate record without value for name field
        """
    ...
    def unlink(self, cr, uid, ids, context=None):
        """
        This method prevent record deletion if it Referenced
        """
...
class hr_violation_punishment(osv.Model):
    ...
    _description = "punishments for Violations"
...
class hr_employee_violation(osv.Model):
    ...
    def _positive_factor(self, cr, uid, ids, context=None):
        """
        Constrain method  to check if factor & penalty_amount value more than zero 
    ...
    def check_dates(self, cr, uid, ids, context=None):
        """
        Constrain method  to check if Exception violation-date lower then Exception decision-date.
        return True or False
        """        
    ...
    def check_dates2(self, cr, uid, ids, context=None):
        """
        Constrain method  to check if start_date greater then end_date.
        return True or False
        """
    ...
    def check_dates3(self, cr, uid, ids, context=None):
        """
        Constrain method  to check if violation_date greater than start_date.
        return True or False
        """
    ...
    _constraints = [
        ...
        (check_dates3, 'Error! Exception violation-date must be lower then Exception procedural start-date.', ['violation_date', 'start_date']),
    ]
    ...
    def onchange_punishment(self, cr, uid, ids, punishment_id, context=None):
        ...
        """
        penalty=punishment_id and self.pool.get('hr.punishment').browse(cr, uid, punishment_id, context=context).penalty or False
        bounse=punishment_id and self.pool.get('hr.punishment').browse(cr, uid, punishment_id, context=context).bonus or False
        degree=punishment_id and self.pool.get('hr.punishment').browse(cr, uid, punishment_id, context=context).degree or False
        return {'value': {'penalty':penalty,'degree':degree,'bounse':bounse}}
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        @return: Super unlink method or raise exception
        """
...
class hr_employee_delegation(osv.Model):
    """
    Inherits hr.employee.delegation and adds function to check if the employee has punishment before delegating  him.
    """
...
class hr_employment_termination(osv.Model):
    """
    Inherits hr.employment.termination and adds function to check if the employee has punishment before terminating his service.
    """
...
class employee_movements_promotion(osv.Model):
    ...
    def do_approve(self, cr, uid, ids, context=None):
         for emp in self.browse(cr, uid, ids, context=context):
                emp_obj = self.pool.get('hr.employee.violation')
                emp_id= emp_obj.search(cr, uid, [('employee_id','=',emp.employee_id.id),('end_date','>=',emp.approve_date),('start_date','<=',emp.approve_date),('degree','=',True),('state','=','implement')], context=context)
                if emp_id:
                    raise osv.except_osv(_('Warrning'), _('This employee has penalty not finished yet!'))
         return super(employee_movements_promotion , self).do_approve(cr , uid , ids , context)
...
class employee_movements_bonus(osv.Model):
    ...
    def do_approve(self, cr, uid, ids, context=None):
         for emp in self.browse(cr, uid, ids, context=context):
                emp_obj = self.pool.get('hr.employee.violation')
                emp_id= emp_obj.search(cr, uid, [('employee_id','=',emp.employee_id.id),('end_date','>=',emp.approve_date),('start_date','<=',emp.approve_date),('bounse','=',True),('state','=','implement')], context=context)
                if emp_id:
                    raise osv.except_osv(_('Warrning'), _('This employee has penalty not finished yet!'))
         return super(employee_movements_bonus , self).do_approve(cr , uid , ids , context)    
