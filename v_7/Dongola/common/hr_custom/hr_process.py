...
class hr_promotion_type(osv.osv):
    ...
    _columns = {
        ...
        'margin_time':fields.integer('Days Number',size=256,required=True),
    } 
...
class hr_process_archive(osv.osv):
    ...
    _columns = {
        'name': fields.char("Name", size=64 , states={'approved':[('readonly',True)]}),
    ...
    def create(self, cr, user, vals, context=None):
        if type(vals['reference']) == int :
            return super(hr_process_archive, self).create(cr, user, vals, context)
        if ('name' not in vals) or (vals.get('name')=='/'):
            (seq_obj_name, id) = vals['reference'].split(',') 
            vals['name'] = self.pool.get('ir.sequence').get(cr, user, seq_obj_name)
        new_id = super(hr_process_archive, self).create(cr, user, vals, context)
        return new_id 
    ...
    def onchange_employee(self, cr, uid, ids, emp_id,context={}):
        """
        Onchange employee method to set the previous job of the employee or its previous 
        department based on the selected employee.
    ...
    def onchange_reference(self, cr, uid, ids, reference, employee_id, context=None):
        ...
        department based on the selected reference.
    ...
    def create_new(self, cr, uid, ids,context=None):
        ...
        changes employee's job and writes the date of changing if the reference is hr.job or 
        changes employee's department if the reference is hr.department
        @return: boolean True 
        """
    ...
    _constraints = [
        (_check_reference, "You can not choose an employee's current job/department!", []),
    ]  
...
class hr_employee_delegation(osv.Model):
    ...
    _constraints = [
        (_check_date, 'You can not have 2 delegation that overlaps on same day!', []),
    ]
    ...
    def my_done(self, cr, uid, ids,context=None):
        ...
        for rec in self.browse(cr, uid , ids):
            ...
            emp_obj.write(cr, uid, [rec.employee_id.id], {'delegation':False ,'state':'approved','end_date' : False})
    ...
    def approved(self, cr, uid, ids,context=None):
        ...
        for rec in self.browse(cr, uid , ids):
            ...
            if rec.type == 'transferred':
                delegation = False
                emp_obj.write(cr, uid, [rec.employee_id.id], {'end_date':rec.start_date})
                wf_service.trg_validate(uid, 'hr.employee',rec.employee_id.id ,'refuse', cr)
    ...
    def action_draft(self, cr, uid, ids,context=None):
        ...
        for rec in self.browse(cr, uid , ids):
            delegation = False
            if rec.type == 'transferred':
                emp_obj.write(cr, uid, [rec.employee_id.id], {'end_date':False})
                wf_service.trg_validate(uid, 'hr.employee',rec.employee_id.id ,'approve', cr)
...
class hr_employee_reemployment(osv.Model):
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid,ids):
            ...
            emp_obj.write(cr, uid, [record.employee_id.id], {'re_employment_date': record.reemployment_date,'end_date': False} , context=context)
            ...
            #wf_service.trg_delete(uid, 'hr.employee', record.employee_id.id, cr)
            wf_service.trg_validate(uid, 'hr.employee',record.employee_id.id ,'approve', cr)
            vals= {
