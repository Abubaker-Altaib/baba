...
class report_hr_job(report_sxw.rml_parse):
    ...
    def get_info(self,data):
        if  data['form']['report_type'] == 'jobs':
            self.info['title']=u"تقرير الوظائف"
        else:
            self.info['title']=u"مدراء الادارات و الاقسام"
    ...
    def create_xml(self, data):
        append_where = "  "
        if  data['form']['report_type'] == 'jobs':
            label = {u'a1': u'القسم', u'a2': u'المصدق',u'a3': u'المشغول',u'a4': u'الشاغر','level':4}                         
            ...
            self.cr.execute(''' SELECT 2 as level, j.id as id, j.name as a1, no_of_recruitment as a2 ,
                                no_of_employee as a3, expected_employees as a4   
                                FROM   hr_job as j 
            ...
            label = {'a1': u'الإدارة / القسم','a2': u'المدير', 'a3':u'الرقم', 'level':4}  
            append_where = data['form']['department_ids'] and \
                    " where d.id in (%s) " % ','.join(map(str, data['form']['department_ids'])) or "  "
            self.cr.execute("  SELECT 3 as level, d.name as a1, r.name as a2, h.emp_code as a3 FROM\
                    hr_department d \
