...
class hr_job(osv.osv):
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if 'allow_job_ids' in context:
            allow_id = context['allow_id']
            allow_job_ids = context.get('allow_job_ids' , [])
            pool_obj = self.pool.get('hr.allowance.job')
            res_ids = pool_obj.search(cr , uid , [('allow_deduct_id' , '=' , allow_id)])
            res_data = pool_obj.read(cr , uid , res_ids , ['job_id'])
            job_ids = [i['job_id'][0] for i in res_data]
            if allow_job_ids:
                for i in allow_job_ids:
                    if i[2]:
                        job_id = i[2].get('job_id' , False)
...
class hr_employee(osv.osv):
    ...
    _columns = {
        ...
        'location_id' : fields.many2one('hr.department', string="Location"), 
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        """
        if vals['employee_type'] =='contractor':
           vals['lang']='ar_SY'
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        #update_field = [key for key in vals.keys() if key in ('payroll_id', 'degree_id', 'bonus_id', 'status', 'state', 'tax_exempted', 'category_ids', 'company_id', 'substitution')]
        if update_field:
...
class employee_family(osv.osv):
    ...
    def family_approved(self, cr, uid, ids, context=None):
        ...
        return super(employee_family, self).family_approved(cr, uid, ids, context)
    ...
    def family_stopped(self, cr, uid, ids, context=None):
        """
        ...
        for relation in self.browse(cr, uid, ids, context=context):
            ...
            if relation.change_marital == True:                   
                employee_obj.write(cr, uid, [relation.employee_id.id], {'marital': relation.marital})                
        self.write(cr, uid, ids, vals, context=context)
        employee_obj.write_employee_salary(cr, uid, [relation.employee_id.id], [])
        return True
    ...
    def set_to_draft(self, cr, uid, ids, context=None):
        ...
        return True
    def family_rejected(self, cr, uid, ids, context=None):
        """Workflow function that changes the state to 'rejected' and re-calculates the salary.
           @return: Boolean True
        """
        employee_obj = self.pool.get('hr.employee')
...
class hr_employment_termination(osv.Model):
    ...
    def calculation(self, cr, uid, ids, transfer , context=None):
        ...
        for rec in self.browse(cr, uid, ids, context=context):
            ...
            if deduct:
                for d in deduct:
                    current_date = mx.DateTime.Parser.DateTimeFromString(rec.dismissal_date)
                    if not d.end_date:
                       raise orm.except_orm(_('Error'), _('Please Review the Employee Allowance Dedection Exception')) 
                    end_date = mx.DateTime.Parser.DateTimeFromString(d.end_date) 
                    emp_end_date_days = (end_date - current_date).days
                    day=d.amount/30
        ...
        for allow in allowance_ids:
            emp = self.pool.get('hr.employee').browse(cr, uid, rec.employee_id.id, context=context)
            #amount = payroll.compute_allowance_deduction(cr, uid, rec.employee_id, allow.id)(cr,uid,start_date,emp,{'no_sp_rec':True}, [ punishment.allow_deduct.id], False,[])
            amount = payroll.allowances_deductions_calculation(cr, uid,rec.dismissal_date,emp,{}, [allow.id],False,[])
    ...
    def transfer(self, cr, uid, ids, context=None ):
        ...
        for rec in self.browse(cr, uid, ids):
            ...
            for line in rec.line_ids:
                line = {
                  'name' :line.name,
                  'allow_deduct_id':line.allow_deduct_id.id,
                  'amount':line.amount,
...
class hr_employee_delegation(osv.osv):
    ...
    _columns = {
        'payroll_type':fields.selection([('paied', 'Paied'), ('unpaied', 'Unpaied'),
                                         ('customized', 'Customized')], 'Payroll',readonly=True, states={'draft':[('readonly',False)]}),
...
class hr_employee_category(osv.Model):
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if 'allow_cat_ids' in context:
            allow_id = context['allow_id']
            allow_cat_ids = context.get('allow_cat_ids' , [])
            pool_obj = self.pool.get('hr.allowance.cat')
            res_ids = pool_obj.search(cr , uid , [('allow_deduct_id' , '=' , allow_id)])
            res_data = pool_obj.read(cr , uid , res_ids , ['cat_id'])
            cat_ids = [i['cat_id'][0] for i in res_data]
            print "cat .......... " , cat_ids
            if allow_cat_ids:
                for i in allow_cat_ids:
                    if i[2]:
                        cat_id = i[2].get('cat_id' , False)
                        if cat_id : cat_ids.append(cat_id)
...
#----------------------------------------
#HR Departments(inherit) 
#----------------------------------------
class hr_department(osv.Model):

    _inherit = "hr.department"
    _columns = {
        'payroll_use' : fields.boolean('Use in Payroll Report') ,
    }
    _defaults = {
        'payroll_use' : False,
    }
    ...
    def name_search(self, cr, uid, name, args=None, operator='ilike', context=None, limit=100):
        ...
        if context.get('type' , False) == 'location':
            if cat_ids:
                args.append(('cat_id' , 'in' , cat_ids ))
            else :
                args.append(('id' , '=' , -1 ))
        else :
            args.append(('cat_id' , 'not in' , cat_ids))
...
class hr_department_cat(osv.Model):
    ...
    _columns = {
        'outsite_scale' : fields.boolean('Set as Location (Outsite Scale)') ,
    }
    _defaults = {
        'outsite_scale' : False,
    }
