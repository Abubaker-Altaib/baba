...
import mx
from dateutil.rrule import DAILY , rrule ,SU,MO,TU,WE,TH
...
class payroll(osv.osv):
    ...
    def family_relation_calculation(self, cr, uid, emp_id, date,include_stop=False):
       ...
       if include_stop:
		...
		family = family_pool.read_group(cr, uid, [
		         ...
		         ('state', 'in', states)
		], ['relation_id'], ['relation_id'],context=context)

		for m in family:
		    relation=relation_pool.browse(cr, uid,[m['relation_id'][0]] ,context=context)[0]
		...
		return {'rel_amount':rel_amount, 'child_no':child_no}
	       ...
	       rel_ids = family_relation_obj.search(cr, uid, [('employee_id', '=', emp_id), ('start_date', '<=', date), ('state', 'in',      states)],context=context)
	       if rel_ids:
		  for rel in family_relation_obj.browse(cr, uid, rel_ids,context=context):
		     if not rel.end_date or rel.end_date > date :
			if rel.relation_id.max_age >= 0 and rel.relation_id.children:
			   # relation with max age is son relation
                           first_end=rel.end_date
			   if rel.birth_date:
			      ...
			      if age <= rel.relation_id.max_age or first_end>today_dt:
			         ch_no += 1
			         if ch_no > rel.relation_id.max_number:
			            ch_no= rel.relation_id.max_number 
			         rel_amount = ch_no*rel.relation_id.amount
			     
			elif rel.relation_id.max_age >= 0 and not rel.relation_id.children:
			   # relation without max age is wife relation
			   wife_amount = rel.relation_id.amount
		  rel_amount=rel_amount+wife_amount
	       return {'rel_amount':rel_amount, 'child_no':ch_no}
        ...
        if res:
           ...
           if emp_holidays_ids: 
              for holiday in emp_holiday_obj.browse(cr,uid,emp_holidays_ids):
                       ...
                       start_days=datetime(date_f.year,date_f.month,date_f.day)
                       end_days=datetime(date_t.year,date_t.month,date_t.day)
                       work_days=list(rrule(DAILY,dtstart=start_days,until=end_days,byweekday=(SU,MO,TU,WE,TH)))
                       #print "WORK Day>>>>>>>>>>",work_days
                       #print "WORK Day>>>>>>>>>>",len(work_days)
                       #if holiday end in the current date
                       if date_t.month == curr_date.month and date_t.year == curr_date.year:
                          #print "2>>>>>>>>>>>>>>>>>>"
                          #Start And End in the Same month so Take the all holiday days
                          if date_f.month == curr_date.month and date_f.year == curr_date.year:
                             #print "3>>>>>>>>>>>>>>>>>>"
                             #days+= holiday.number_of_days_temp
                             days+=len(work_days)
                             #print "first Days>>>>>>>>>>>>>",holiday.number_of_days_temp
                          #Start In Previous Month
                             ...
                             #print "4>>>>>>>>>>>>>>>>>>"
                             #if create holiday after salary calaculate take all holiday days
                             """if holiday.create_date > prev_salary_date.salary_date:
                                #print "5>>>>>>>>>>>>>>>>>>",prev_salary_date.salary_date
                                days+= holiday.number_of_days_temp"""
                             #else:
			     #print "6>>>>>>>>>>>>>>>>>>",date_t.day
			     start_days=datetime(date_t.year,date_t.month,1)
			     end_days=datetime(date_t.year,date_t.month,date_t.day)
			     work_days=list(rrule(DAILY,dtstart=start_days,until=end_days,byweekday=(SU,MO,TU,WE,TH)))
			        #days+=date_t.day
			     days +=len(work_days)
			        #no need
                       ...
                       # If End In Next Month 
                          ...
                          #print "8>>>>>>>>>>>>>>>>>>"
                          # If Start In Current Month=>Take work day from holiday start date and last day in started month
                          if date_f.month == curr_date.month and date_f.year == curr_date.year:
                             #print "9>>>>>>>>>>>>>>>>>>"
                             start_days=datetime(date_f.year,date_f.month,date_f.day)
                             end_days=datetime(date_f.year,date_f.month,30)   
                             #days+=(30-date_f.day+1)
                             days +=len(work_days)
                          #If Start In Previous Month=>Take all work day in current month
                          elif (date_f.month < curr_date.month and date_t.year == curr_date.year) or (date_f.month >= curr_date.month and date_f.year == curr_date.year-1):
                             #print "10>>>>>>>>>>>>>>>>>>" 
                             """if holiday.create_date < prev_salary_date.salary_date :
                                #print "11>>>>>>>>>>>>>>>>>>"
                                ...
                                #days+=30+ prev_month
                             days +=22
                       elif ((date_t.month < curr_date.month and date_t.year== curr_date.year) or (date_t.month > curr_date.month and date_t.year == curr_date.year-1)):
                            #print "15>>>>>>>>>>>>>>>>>>"
                            #days+=holiday.number_of_days_temp
                            days+=0
           amount_per_day=amount/22
           #print "in abs calculate days>>>>>>>>>>>>>>>>>>>>",days
           deduct=amount_per_day * days
           remain_amount=0.0
           #print "in abs calculate amount>>>>>>>>>>>>>>>>>>>>",deduct
    ...
    def allowances_deductions_calculation(self, cr, uid, date, employee_obj, emp_dict, allow_deduct, substitution=False, allow_list=[]):
        ...
        if not substitution:
            ...
            substitue_ids = employee_substitution_obj.search(cr, uid, ['|', ('end_date', '>=', date), ('end_date', '=', False), 
                            ('employee_id', '=', emp_dict['emp_id']), ('start_date', '<=', date)], context = {'rules':True})
            if substitue_ids:
                for sub_record in employee_substitution_obj.browse(cr, uid, substitue_ids):
                    ...
                    if scale['sub_salary'] == 'sustitut_degree' or 'sustitut_degree' in emp_dict.keys():
        ...
        for record in salary_allow_deduct_obj.browse(cr, uid, allow_deduct_ids):
            ...
            if (substitution and (sub_setting['sub_setting'] and (sub_setting['sub_setting'] == 'first' and \
                   ...
                   record.allow_deduct_id.name_type != 'a') or not substitution:
    ...
    def allowances_deductions_amount(self, cr, uid, emp_dict, allow_list):
        ...
        if allow_deduct.special and not emp_dict['special']:
            ...
            exclusion = allow_deduct_exception_obj.search(cr, uid, ['|', ('end_date', '>', emp_dict['date']), ('end_date', '=', False),
                           ...
                           ('allow_deduct_id', '=', allow_deduct.id), ('action', '=', 'exclusion')],context = {'rules':True})
            if not exclusion:
                ...
                if check :
                  if not allow_deduct.allow_cat_ids and not allow_deduct.allow_job_ids :
                    ...
                    allow_cat_ids = allow_cat_obj.search(cr , uid , [('cat_id' , 'in' , emp_cat_ids) , ('allow_deduct_id' , '=' , allow_deduct.id)], context = {'rules':True})
                    if allow_cat_ids :
                      values = allow_cat_obj.read(cr , uid , allow_cat_ids , ['value'], context = {'rules':True})
                      percent = percent * values[0]['value'] / 100
                    else :
                      allow_job_obj = self.pool.get('hr.allowance.job')
                      allow_job_ids = allow_job_obj.search(cr , uid , [('job_id' , '=' , emp_dict['job_id']) , ('allow_deduct_id' , '=' , allow_deduct.id)], context = {'rules':True})
                      if allow_job_ids :
                        values = allow_job_obj.read(cr , uid , allow_job_ids , ['value'], context = {'rules':True})
                        percent = percent * values[0]['value']  / 100
                      else :
                        check = False
                if check:
                    if allow_deduct.salary_included:
                        amount += emp_dict['basic_salary'] or 0
                        ...
                        if not emp_dict['substitution']:
                            if 'substitution_obj' in emp_dict:
                                if emp_dict['substitution_setting'] == 'diff':
                                    ...
                                    curr_allow_dict['total_allow'] += emp_curr_dict['started_section']
                                    ...
                                    sub_allow_dict['total_allow'] += emp_sub_dict['started_section']
                                    substitution_amount = (sub_allow_dict['total_allow']) - (curr_allow_dict['total_allow'])
                                    #print "Substitute Amount>>>>>>>>>>##",substitution_amount
                    ...
                    if allow_deduct.type == 'amount':
                        ...
                        complex_salary_ids = salary_allow_deduct_obj.search(cr, uid, [
                                                    ...
                                                    ('payroll_id', '=', emp_dict['payroll']), ('degree_id', '=', emp_dict['degree'])], context = {'rules':True})
                        if complex_salary_ids :
                            emp_comp_dict = emp_dict.copy()
                            for comp in salary_allow_deduct_obj.browse(cr, uid, complex_salary_ids, context = {'rules':True}):
                                #if comp.allow_deduct_id.id in allow_list:
                                emp_comp_dict.update({'allow_deduct':comp})
                                emp_comp_dict.update({'in_compose': True})
                    ...
                    if allow_deduct.related_marital_status:
                        family_relation=self.family_relation_calculation(cr, uid, emp_dict['emp_id'], emp_dict['date'],include_stop=True)
                        married=False
                        ...
                        status_ids = marital_status_obj.search(cr, uid, [
                                        ...
                                        ('married', '=', married),('children_no', '=', children)], context = {'rules':True})
                        if status_ids:
        ...
        if check and allow_deduct.related_gender_type and allow_deduct.related_gender_type :
                        ...
                        allow_gender_ids = allow_gender.search(cr , uid , [('gender' , '=' , emp_dict['gender']) , ('allow_deduct_id' , '=' , allow_deduct.id)], context = {'rules':True})
                        values = allow_gender.read(cr , uid , [allow_gender_ids[0]] , ['value'],context = {'rules':True})
                        value = values[0]['value']
                        amount = (amount * value) / 100
        #print "At end>>>>>>",amount
        return {'amount':abs(amount), 'tax':tax_amount ,'holiday_amount':holiday_amount , 'remain_amount' : remain_amount,'imprint': imprint}
    ...
    def allowances_deductions_sp_calculation(self, cr, uid, emp_dict, allow_list):
        ...
        special_ids = allow_deduct_exception_obj.search(cr, uid, ['|', ('end_date', '>', emp_dict['date']), 
                         ...
                         ('employee_id', '=', emp_dict['emp_id']),('start_date', '<=', emp_dict['date']), ('action', '=', 'special')], context = {'rules':True})
        if special_ids and not emp_dict['no_sp_rec']:
            for special in allow_deduct_exception_obj.browse(cr, uid, special_ids):
                if special.allow_deduct_id.in_salary_sheet or emp_dict['in_compose']:
                    if special.amount:
    ...
    def write_allow_deduct(self, cr, uid, emp_id, result_dict , emp_obj=False ):
        ...
        for res in result_dict:
            ...
            check_allow = employee_salary_obj.search(cr, uid, [('employee_id', '=', emp_id), 
                                                               ('allow_deduct_id', '=', res['allow_deduct'].allow_deduct_id.id)],context = {'rules':True})
            ...
            if check_allow:
                if res['amount'] > 0:
                    employee_salary_obj.write(cr, uid, check_allow, allow_deduct_dict,context = {'rules':True})
                else:
                    employee_salary_obj.unlink(cr, uid, check_allow,context = {'rules':True})
            else:
                if res['amount'] > 0:
                    employee_salary_obj.create(cr, uid, allow_deduct_dict,context = {'rules':True})
        employee_salary_obj.unlink(cr, uid, unlink_ids,context = {'rules':True})
        return True
    ...
    def create_payment(self, cr, uid, ids, vals = {}, context = None):
        ...
        for line in lines:
            ...
            if 'account_analytic_id' in keys:
                ...
                account_analytic_id = allow_deduct.analytic_id and allow_deduct.analytic_id.id or None
            line = {
                'name':name ,
