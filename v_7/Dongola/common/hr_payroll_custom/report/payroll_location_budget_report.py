...
class payroll_location_report(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        super(payroll_location_report, self).__init__(cr, uid, name, context)
...
hr_payroll_main_archive.department_id as ids,
hr_department.name AS depart_name,
(select sum(net) from hr_payroll_main_archive where hr_payroll_main_archive.department_id=hr_department.id and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True GROUP BY hr_department.name) AS total ,
(select sum(net) from hr_payroll_main_archive where hr_payroll_main_archive.department_id=hr_department.id and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True GROUP BY hr_department.name) AS prev_total ,
((select sum(net) from hr_payroll_main_archive where hr_payroll_main_archive.department_id=hr_department.id and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True GROUP BY hr_department.name) - (select sum(net) from hr_payroll_main_archive where hr_payroll_main_archive.department_id=hr_department.id and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True GROUP BY hr_department.name)) AS diff 
 FROM
 hr_payroll_main_archive
left join hr_department ON (hr_payroll_main_archive.department_id = hr_department.id)
WHERE hr_payroll_main_archive.company_id =%s and ( (hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s) or (hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s) ) and hr_payroll_main_archive.in_salary_sheet=True''' ,
 (int(data['form']['month']),data['form']['year'],prev_month,year,int(data['form']['month']),data['form']['year'],prev_month,year,c,int(data['form']['month']),data['form']['year'],prev_month,year,))    
...
(b['ids'],prev_month,year,b['ids'],int(data['form']['month']),data['form']['year'],b['ids'],prev_month,year,b['ids'],int(data['form']['month']),data['form']['year']))
                    ...
                    ################tax#################################
                    self.cr.execute('''SELECT distinct
(select distinct hr_payroll_main_archive.tax FROM hr_payroll_main_archive where hr_payroll_main_archive.department_id=%s and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True) as deduct_prev,

(select distinct hr_payroll_main_archive.tax FROM hr_payroll_main_archive where hr_payroll_main_archive.department_id=%s and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True)as deduct_now

FROM hr_allowance_deduction_archive ''',
(b['ids'],prev_month,year,b['ids'],int(data['form']['month']),data['form']['year']))
                    res_tax = self.cr.dictfetchall()
                    #####################################################
                    #print "deeeeeeept>>>>",b['ids']
                    ################basic salary#################################
                    self.cr.execute('''SELECT distinct 
hr_department.name AS depart_name,
(select sum(basic_salary) from hr_payroll_main_archive where hr_payroll_main_archive.department_id=hr_department.id and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True GROUP BY hr_department.name) AS salary_prev ,
(select sum(basic_salary) from hr_payroll_main_archive where hr_payroll_main_archive.department_id=hr_department.id and hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s and hr_payroll_main_archive.in_salary_sheet=True GROUP BY hr_department.name) AS salary_now
FROM
 hr_payroll_main_archive
left join hr_department ON (hr_payroll_main_archive.department_id = hr_department.id)
WHERE hr_payroll_main_archive.company_id =%s and ( (hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s) or (hr_payroll_main_archive.month=%s and hr_payroll_main_archive.year=%s) ) and hr_payroll_main_archive.in_salary_sheet=True 
 ''',
(prev_month,year,int(data['form']['month']),data['form']['year'],c,int(data['form']['month']),data['form']['year'],prev_month,year))
...
(b['ids'],int(data['form']['month']),int(data['form']['month']),data['form']['year'],data['form']['year']))
                    ...
                    if comments:
                        ...
                        comm = comments[1]"""
                    x={
                        'allow_incres':abs(allow_incres),
                        'allow_decres':abs(allow_decres),
                        'deduct_incres':abs(deduct_incres),
                        'deduct_decres':abs(deduct_decres),
                        'depart_name':b['depart_name'],
                        'total':b['total'],
                        'prev_total':b['prev_total'],
                        #'incras':comm,
                    ...
                    if (count % 16) == 0.0 and page == 0:
                        ...
                        count = 0
                        x1={
                        'allow_incres':abs(globals()['total_allow_inc']),
                        'allow_decres':abs(globals()['total_allow_dec']),
                        'deduct_incres':abs(globals()['total_deduct_ince']),
                        'deduct_decres':abs(globals()['total_deduct_dec']),
                        'depart_name':u'الإجمالي',
                        'total':abs(globals()['total_curr']),
                        'prev_total':abs(globals()['total_prev']),
                        #'incras':comm,
                        'no':'-',
...
WHERE hr_payroll_main_archive.company_id =%s''' ,
 (int(data['form']['month']),data['form']['year'],prev_month,year,c))    
        ...
        x={
            'total_allow_inc':abs(globals()['total_allow_inc']),
            'total_allow_dec':abs(globals()['total_allow_dec']),
            'total_deduct_ince':abs(globals()['total_deduct_ince']),
            'total_deduct_dec':abs(globals()['total_deduct_dec']),
            'inc_total':abs(globals()['total_curr']),
            'dec_total':abs(globals()['total_prev']),
...
report_sxw.report_sxw('report.payroll.location.budget', 'hr.payroll.main.archive', 'addons/hr_ntc_custom/report/payroll_location_budget_report.rml' ,parser=payroll_location_report, header=False)
