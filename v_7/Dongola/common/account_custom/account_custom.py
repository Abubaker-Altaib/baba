...
class account_account_type(osv.Model):
    ...
    _columns = {
            ...
            'close_method': fields.selection([('none', 'None'), ('balance', 'Balance'), ('pl', 'Reset'), ('detail', 'Detail'), 
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        """
        Inherit copy method to add '(copy)' term to the copied type's name 
        """
        if context is None:
            context = {}
        if default is None:
...
def _code_check(self, cr, uid, ids, context=None):
        """
        Constraint method that check if account code consist of only numbers
        
        @return: boolean
        """
        for acc in self.browse(cr, uid, ids, context=context):
            if not re.match(r'^[0-9]*$', acc.code):
                return False
        return True

    _constraints = [
         (_code_check, "You have unsupported characters in your account code! account code should be numeric!", ['code']), 
    ]
'''
...
class account_journal(osv.Model):
    ...
    Inherit account journal to :
        * Add special field 
        * Change property of user_id field from many2one to many2many
    ...
    def create(self, cr, uid, vals, context=None):
        """
        Inherit create method to convert user_id value from int to many2many format
...
class account_move(osv.Model):
    ...
    _columns = {
        'state': fields.selection([('draft', 'Draft'), ('completed', 'Completed'), 
                 ('closed', 'Closed'), ('posted', 'Posted'), ('analytic', 'Analytic Check'), 
                 ('reversed', 'Reverse'), ('to_review_closer', 'To Review(Closer)'), 
                 ('to_rev_manager', 'To Review(Manager)'), ('review_analytic', 'To Review(analytic)')], 
    ...
    def _required_line_id(self, cr, uid, ids, context=None):
        """
        Constraint method make sure that not draft moves has lines
    ...
    def _debit_credit_check(self, cr, uid, ids, context=None):
        """
        Constraint method make sure that all not draft moves lines has positive number in debit or credit fields
    ...
    def check_completed(self, cr, uid, ids, context=None):
        """
        Workflow function change record state to 'to_review_closer'.
        
        @return: boolean True
        """
        return self.write(cr, uid, ids, {'state': 'to_review_closer'}, context=context)

    def closed(self, cr, uid, ids, context=None):
        """
        Workflow function change record state to 'closed'.
        
        @return: boolean True
        """
        return self.write(cr, uid, ids, {'state': 'closed'}, context=context)

    def check_closed(self, cr, uid, ids, context=None):
        """
        Workflow function change record state to 'to_rev_manager'.
        
        @return: boolean True
        """
        return self.write(cr, uid, ids, {'state': 'to_rev_manager'}, context=context)
        ...
        for mv in self.browse(cr, uid, ids, context=context):
            for line in mv.line_id:
                if line.account_id.user_type.analytic_required and not line.analytic_account_id and line.debit:
                    raise orm.except_orm(_('Error!'), _('You must add analytic accountt for %s accounts!')%(line.account_id.user_type.name,))
        return self.write(cr, uid, ids, {'state':'completed'}, context=context)

    def check_analytic(self, cr, uid, ids, context=None):
        """
        Workflow function change record state to 'to_rev_manager'.
        
        @return: boolean True
        """
        return self.write(cr, uid, ids, {'state': 'review_analytic'}, context=context)
    ...
    def revert_move(self, cr, uid, ids, journal, period, date, reconcile=True, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            copy_id = self.copy(cr, uid, move.id, context=context)
            self.write(cr, uid, [copy_id], {'ref': context.get('ref', '')+move.name, 'journal_id': journal, 'period_id': period, 'date': date}, context=context)
...
class account_move_line(osv.Model):
    ...
    def create(self, cr, uid, vals, context=None, check=True):
        ...
        if context.get('reverse_move', False):
            ...
            vals.update({'debit': credit, 'credit': debit})
    ...
    def _update_check(self, cr, uid, ids, context=None):
        """
        Method that prevent updating posted moves and moves with reconciled lines
    ...
    def _default_get(self, cr, uid, fields, context=None):
        """ 
        This Method get the default value for move line 
    ...
    def onchange_account_id(self, cr, uid, ids, account_id=False, partner_id=False, context=None):
        ...
        if account_id:
            ...
            if res.user_type.analytic_required :
                dummy, analytic_account_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'account_custom', 'normal_analytic_account')
                if analytic_account_obj.search(cr,uid,[('id','=',analytic_account_id)]):
                    val['analytic_account_id'] = analytic_account_id
        return {'value': val}
...
class account_analytic(osv.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for r in self.browse(cr, uid, ids, context=context):
            name = ''
            try:
                name += r.company_id.code and r.company_id.code+'-' or ''
            except:
                pass
            try:
                name += r.code and r.code+' ' or ''
            except:
                pass
...
class res_partner(osv.Model):
    ...
    def copy_data(self, cr, uid, id, default=None, context=None):
        """
        Inherit copy_data method to add '(copy)' term to the copied partner name
...
class account_fiscalyear(osv.Model):
    ...
    def _periods_check(self, cr, uid, ids, context=None):
        """
        Constraint method make sure that not draft fiscal year has periods
        
        @return: boolean 
        """
        if self.search(cr, uid,[('id', 'in', ids),('period_ids', '=', False),('state', '!=', 'draft')], context=context):
            return False
        return True

    def _check_overlapping(self,cr,uid,ids,context=None):
        """
        Constraint method prevent overlapping fiscal years dates
        
        @return: boolean 
        """
        for fiscalyear in self.browse(cr, uid, ids, context=context):
            if self.search(cr, uid, [('date_stop','>=',fiscalyear.date_start),('date_start','<=',fiscalyear.date_stop),('company_id','=',fiscalyear.company_id.id),('id','<>',fiscalyear.id)],context=context):
                    return False
        return True
    
    _constraints = [
         (_periods_check, "Fiscal year should has at least one period!", []),
         (_check_overlapping, 'Error!\nFiscal years shouldn\'t overlap.', ['date_start','date_stop'])
    ]

    def unlink(self, cr, uid, ids, context=None):
        """
        Inherit unlink method to prevent delete not draft fiscal years
        
        @return: super unlink
        """
        if self.search(cr, uid,[('id', 'in', ids),('state', '!=', 'draft')], context=context):
            raise orm.except_orm(_('UserError'), _('You can\'t delete not draft fiscal year!'))
...
class account_period(osv.Model):
    ...
    def find(self, cr, uid, dt=None, context=None):
        """
        Inherit find method to return only open period 
        
        @return: int period id or raise an exception
        """
        if context is None:
            context = {}
    ...
    def action_close(self, cr, uid, ids, context=None):
        ...
        @param ids: account period close’s ID or list of IDs
         """
        mode = 'done'
        account_move_ids = self.pool.get('account.move').search(cr, uid, [('period_id', 'in', ids), ('state', '=', "draft")], context=context)
        if account_move_ids:
            raise osv.except_osv(_('Invalid Action!'), _('In order to close a period, you must first post related journal entries.'))
        #self.pool.get('account.journal.period').write(cr, uid, {'state': 'done'}, context=context)
        #self.write(cr, uid, {'state': 'done'}, context=context)
...
class account_invoice_line(osv.Model):
    ...
    def onchange_account_id(self, cr, uid, ids, product_id, partner_id, inv_type, fposition_id, account_id):
        ...
        analytic_account_id_val = False
        if account.user_type.analytic_required :
            analytic_account_obj = self.pool.get('account.analytic.account')
            dummy, analytic_account_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'account_custom', 'normal_analytic_account')
            if analytic_account_obj.search(cr,uid,[('id', '=', analytic_account_id)]):
                analytic_account_id_val = analytic_account_obj.search(cr,uid,[('id', '=', analytic_account_id)])
                analytic_id = analytic_account_obj.browse(cr, uid, analytic_account_id_val[0]).id
        ...
        res = {'value':{'invoice_line_tax_id': unique_tax_ids,
