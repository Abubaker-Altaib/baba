...
class hr_holidays_status(osv.osv):
    ...
    def get_days(self, cr, uid, ids, employee_id, return_false, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.leave_limit == 'annual' and record.save_leave:
                ...
                all_leaves += all_leaves * save_years
            ...
            if record.check_remain or record.save_remain:
                    ...
                    print grouped_lines_unpaid_annual
                        ...
                        for rec in grouped_lines_unpaid_annual:
                            ...
                            '''else:
                                if rec in Leaves:
                                    last_leave = self.pool.get('hr.holidays.status').browse(cr, uid, rec) 
                                    remain = last_leave.number_of_days + (last_leave.number_of_days * last_leave.save_years)
                                    print remain,"<<<<<<<<,remaining2222222"
                                    for item in grouped_lines_unpaid_annual[rec]:
                                        last_taken_leaves += item['number_of_days_temp']
                                        print last_taken_leaves,"<<<<<<<<<<<<last_leave"
                                        if remain > item['remaining_leaves']:
                                            remain =  item['remaining_leaves']'''
    ...
    def unlink(self, cr, uid, ids, context=None):
        """
        prevent recored delection if its refrenced in employees holidays
        """
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        holiday_type = self.browse(cr, uid, id, context=context)
        if holiday_type.code:
            default.update({'code':"%s (copy)"% (holiday_type.code or '')})
        if holiday_type.name:
            default.update({'name':"%s (copy)"% (holiday_type.name or '')})
        return super(hr_holidays_status, self).copy(cr, uid, id, default=default, context=context)
    ...
    _columns = {
        ...
        'min_days': fields.integer('Minimum no of Days'),
        'annual_programming': fields.boolean('Subservient to Annual Programming'),
        'alternative_emp': fields.boolean('Require Alternative Employee'),
        ...
        'linked_with_unpaid_leaves': fields.boolean('Linked With Unpaid Leaves'),
        'check_remain': fields.boolean('Check Remaining'),
        'save_remain': fields.boolean('Save Remaining'),
        'payroll_type':fields.selection([('paied', 'Paied'), ('unpaied', 'Unpaied'), 
                                         ('customized', 'Customized'),('half','Half'),('third','Third'),('quarter','Quarter')], 'Payroll', required=True)
    ...
    _sql_constraints = [
       ...
       ('min_days_check', 'CHECK (min_days >= 0)', "The Minimum no of Days should be greater than or equal to Zero!"),
    ]
    ...
    '''def name_get(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            name = record.name
            remaining_leaves = record.get_days(context.get('employee_id'), False)[record.id]['remaining_leaves']
            if not record.limit:
                name = name + ('  (%g/%g)' % (record.max_leaves or 0.0,remaining_leaves or 0.0))
...
class hr_holidays(osv.osv):
    ...
    def check_holidays(self, cr, uid, ids, context=None):
        ...
        for holiday in self.browse(cr, uid, ids, context=context):
            ...
            if holiday.holiday_status_id.leave_limit == 'annual':
                ...
                remaining = holiday_details.remaining_leaves
                if not holiday.holiday_status_id.limit and remaining < 0:
    ...
    _columns = {
        ...
        'notes': fields.text('Reasons', states={'refuse':[('readonly',True)], 'confirm':[('required', True)]}),
        'number_hours': fields.float('Numbers Of Hours'),
    }

    _defaults = {
        'employee_id': _employee_get, #To call onchange_employee
        'date_from': lambda *x: time.strftime('%Y-%m-%d %H:%M:%S'),
        'name': '/'
    }
    ...
    _constraints = [
        ...
        (check_holidays, _('holiday days must be  greater than remaining days!'), []),
    ...
    def onchange_holiday(self, cr, uid, ids, holiday_id, employee_id, date_from=None, context=None):
        ...
        return {'value':{'number_of_days_temp': remaining_leaves}}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        holiday_pool = self.pool.get('hr.holidays.status')
        for holiday in self.browse(cr, uid, ids, context=context):
            name = vals.get('holiday_status_id') and holiday_pool.browse(cr, uid, vals['holiday_status_id'], context=context).name or \
    ...
    def onchange_employee(self, cr, uid, ids, emp_id, context=None):
        ...
        if emp_id:
            ...
            domain['holiday_status_id'].append(('permission', '!=', True))
        ...
        domain = {
            ...
            'alternative_employee': employee_domain['employee_id'],
        }
...
class hr_public_events(osv.osv):
    ...
    _constraints = [
        (check_number_of_days, _('The number of event days should be greater than Zero!'), []),
    ]
...
class hr_dismissal(osv.osv):
    ...
    """
    Inherits hr.dismissal and adds boolean field to set whether to close the holiday out with service termination or not
    """
...
class hr_employment_termination(osv.Model):
    ...
    def calculation(self, cr, uid, ids, transfer=False, context=None):
        """
        ...
        super(hr_employment_termination, self).calculation(cr, uid, ids, transfer=transfer, context=context)
        for rec in self.browse(cr, uid, ids, context=context):  
            if rec.dismissal_type.holiday_settlement:
                holi_status_ids = holiday_status.search(cr, uid, [('settlement','=',True),('buy_leave','=',True)])
                if  holi_status_ids:
                    for status in holiday_status.browse(cr, uid, holi_status_ids, context=context) :
                        ...
                        termination_lines.create(cr, uid, {  'account_id': status.buy_allowance_id.account_id.id,
                        ...
                        if transfer == True:
                            ...
                            holiday.write(cr, uid, [cr_holi_id], {'state': 'paid'}, context=context)
        return True
