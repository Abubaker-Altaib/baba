...
class account_budget_object(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        return super(account_budget_object, self).set_context(objects, data, ids, report_type=report_type)
    ...
    def _budget_total(self,budget):
        ...
        for line in budget.account_budget_line:
            ...
            total['confirm']+=line.confirm
            total['balance']+=line.balance
            total['residual_balance']+=line.residual_balance
        return [total]

report_sxw.report_sxw('report.account.account.budget.object1', 'account.budget', 'addons/account_budget_confirmation/report/account_report_budget_obj.rml', parser=account_budget_object, header='internal landscape')
