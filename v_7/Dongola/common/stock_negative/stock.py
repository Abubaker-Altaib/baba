...
class stock_warehouse_orderpoint(osv.osv):
    ...
    _sql_constraints = [
        ('qty_min_check', 'CHECK( product_min_qtye >= 0 )', 'Minimum Quantity must be greater than or equal zero.'),
        ('qty_max_check', 'CHECK( product_max_qty >= 0 )', 'Maximum Quantity must be greater than or equal zero.'),
    ]
class stock_move(orm.Model):
    ...
    def _merge_move(self, cr, uid, move_id, context=None):
        """To merge split move Then when you will check the availability again in this Picking,
           and split moves are available,This will merge these to move into only one"""
        if context is None:
            context = {}
    ...
    def _split_move(self, cr, uid, move, context=None):
        """Split moves when there are some moves which are partially available"""
...
#            else:
    ...
    def action_assign(self, cr, uid, ids, context=None):
        ...
        # procurement
        for move in self.browse(cr, uid, ids + tomerge, context=context):
            if move.state !='assigned':
                proc=procurement_obj.search(cr, uid, [('move_id', '=', move.id)], context=context)
                #if not proc:
                   #procurement_obj.create(cr, uid, self._prepare_order_line_procurement(cr, uid, move, context=context))
        # Merge moves
