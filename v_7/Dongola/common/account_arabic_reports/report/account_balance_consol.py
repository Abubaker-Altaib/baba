...
class account_balance_consol(report_sxw.rml_parse, common_header, custom_common_header):
    _name = 'report.account.account.consol.balance'

    def __init__(self, cr, uid, name, context=None):
        super(account_balance_consol, self).__init__(cr, uid, name, context=context)
        self.sum_debit = 0.00
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(account_balance_consol, self).set_context(objects, data, new_ids, report_type=report_type)
    ...
    def type(self, form, ids=[], done=None):
        ...
        for type in types:
            ...
            if type=='ministry':
                type2='other'
            company_ids= self.pool.get('res.company').search(self.cr, self.uid, [('id', 'child_of', company_id),('type', '=',type2) ], context=self.context)
            account=form['account_ids'][0]
            code=self.pool.get('account.account').browse(self.cr, self.uid,account).code 
            accounts= self.pool.get('account.account').search(self.cr, self.uid, [('code', 'like', code),('company_id', 'in', company_ids)], context=self.context)
            ...
            if date_from:
                    ...
                    fiscalyear_ids = fiscalyear_obj.search(self.cr,self.uid, [ ('code', 'like', f_code)])
    ...
    def company(self, form, type,ids=[], done=None):
        sign=self._get_account_sign(form)
        fiscalyear_obj = self.pool.get('account.fiscalyear')
        fiscalperiod_obj = self.pool.get('account.period')
        types=('locality','ministry')
    ...
    def total(self, form, ids=[], done=None):
        ...
        types=[]
        if form['unit_type']:
           if form['unit_type']=='locality':
              types.append('loc_sub')
           if form['unit_type']=='ministry':
              types.append('other')
            
        else:
           types=('other','loc_sub')
  
        company_ids= self.pool.get('res.company').search(self.cr, self.uid, [('id', 'child_of', company_id),('type', 'in',types) ], context=self.context)
        account=form['account_ids'][0]
        code=self.pool.get('account.account').browse(self.cr, self.uid,account).code 
...
report_sxw.report_sxw('report.account.account.balance.consol.arabic', 'account.account', 'addons/account_arabic_reports/report/account_balance_consol.rml', parser=account_balance_consol , header=True)
