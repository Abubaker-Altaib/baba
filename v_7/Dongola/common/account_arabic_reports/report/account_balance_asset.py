...
class account_balance_asset(report_sxw.rml_parse, common_header, custom_common_header):
    ...
    def lines(self, form, ids=[], done=None):
        ...
        for it in self.result_acc:
          ...
          child_ids = obj_account._get_children_and_consol(self.cr, self.uid, it['id'], ctx)
          self.cr.execute('''select (sum(COALESCE(debit,0)) -sum(COALESCE(credit,0))) as bl from account_move_line s join account_move m on m.id=s.move_id where m.state!='reversed' and  m.date= %s  and s.company_id in %s and s.account_id in %s''',(date_start,tuple(company_ids),tuple(child_ids)))  
          init=self.cr.fetchone()
          if not init[0]:
               init2=0.0
          else:
               init2=init[0]
          ...
          if it['init']!=0:
            it['init']=sign*it['init']
           
          it['init_balance']= it['init_balance']-it['init']
    ...
    def _get_total(self, form, ids=[]):
        ...
        if date_from2==date_start:
            ...
            self.cr.execute('''select sum(COALESCE(debit,0)),sum(COALESCE(credit,0)) from account_move_line s join account_move m on m.id=s.move_id where m.state!='reversed' and  s.date >%s and s.date <= %s  and s.date != %s and s.company_id in %s  and s.account_id in %s''',(date_from,date_to,date_start,tuple(company_ids),tuple(child_ids)))     
            ...
            if not move[1]:
               credit=0.0
                ...
                credit=move[1]
            self.cr.execute('''select (sum(debit)-sum(credit)) as bl from account_move_line s join account_move m on m.id=s.move_id where m.state!='reversed' and  m.date<= %s and  m.date> %s and s.company_id in %s and s.account_id in %s''',(date_from,date_start,tuple(company_ids),tuple(child_ids)))  
            ...
            debit=credit=0
            self.cr.execute('''select sum(COALESCE(debit,0)),sum(COALESCE(credit,0)) from account_move_line s join account_move m on m.id=s.move_id where m.state!='reversed' and  m.date between %s and %s and s.date != %s and s.company_id in %s  and s.account_id in %s''',(date_from,date_to,date_start,tuple(company_ids),tuple(child_ids)))     
