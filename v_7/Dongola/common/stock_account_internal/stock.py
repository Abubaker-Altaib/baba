...
class stock_move(osv.Model):
    ...
    def _create_account_move_line(self, cr, uid, move,date, src_account_id, dest_account_id, reference_amount, reference_currency_id, context=None):
        ...
        partner_id = (move.picking_id.partner_id.id and move.picking_id.partner_id.id and move.picking_id.partner_id.id) or False
...
class stock_inventory(osv.Model):
    ...
    def unlink(self, cr, uid, ids, context=None):
        for inv in self.browse(cr, uid, ids, context=context):
            if inv.state != 'draft': 
                raise osv.except_osv(_('Invalid Action!'), _('You cannot delete  %s stock inventory '%(inv.state,)))
