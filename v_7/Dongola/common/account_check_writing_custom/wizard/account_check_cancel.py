...
class account_cancel_check(osv.osv_memory):
    ...
    def get_moves(self, cr, uid, ids, context=None):
        """
        Wizard action method that cancel out of date checks then display them
         
         @return: dictionary action that that display canceled checks in check log
        """
        if context is None:
            context = {}
    ...
    def get_move(self, cr, uid, ids=False,journal_ids=False, context=None):
        ...
        * change status the check in the log to canceled
        ...
        for log_obj in log_pool.browse(cr, uid, log_ids, context=context):
            try:
                (model, mail_GM_group) = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mail', 'group_general_manager')
                (model, mail_U_group) = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mail', 'group_account_user')
                (model, mail_M_group) = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'mail', 'group_account_manager')
                
                self.pool.get('mail.group').message_post(cr, uid, [mail_GM_group, mail_U_group, mail_M_group],
                    body=_('The check number %s is not delivered , and it is Expiry period !') % (log_obj.check_no),
