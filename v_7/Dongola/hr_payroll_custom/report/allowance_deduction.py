...
class hr_allowance_deduction_report(report_sxw.rml_parse):
    ...
    def process(self, data ,by_id):
        ...
        self.cr.execute(
            ...
            + where_clause +
            'ORDER BY ad.sequence,emp.sequence',(data['form']['month'],data['form']['year'],data['form']['state_id'],tuple([data['form']['company_id'][0]]),by_id)) 
...
'FROM  hr_payroll_main_archive AS p '\
 ...
 'left join hr_employee_loan b on (m.loan_id=b.id) '\
  ...
  'GROUP BY b.name ',(data['form']['month'],data['form']['year'],tuple([data['form']['company_id'][0]]) ,data['form']['state_id']) )
        res = self.cr.dictfetchall()
        return res
    ...
    def _basic(self, data):
        self.cr.execute('SELECT sum(basic_salary) as basic '\
            ...
            'and pm.company_id IN %s and emp.payroll_state=%s ',(data['form']['month'],data['form']['year'],tuple([data['form']['company_id'][0]]),data['form']['state_id'])) 
        res = self.cr.dictfetchall()
        #print">>>>>>>>>>>>>>>>>>" ,res
        return res
