...
class account_cancel_check(models.TransientModel):
    ...
    def get_moves(self):
        ...
        print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>here 1 ",self.to_reconcile , (self.to_reconcile == 'checks')
    ...
    def get_move(self=False,journal_ids=False):
        ...
        * change status the check in the log to canceled
        ...
        print ">>>>>>>>>>>>>>>>>>>>>[journal_ids_list] ",[journal_ids_list]
        ...
        for journal in journal_obj.search([('id','in',journal_ids_list)]):
            ...
            # Cancel Checks Only 
                ...
                log_ids = log_pool.search([('status','=','active'),('name.move_line_ids','in',ml_ids)]).id
                ...
                for log_obj in log_pool.search([('id','in', [log_ids])]):
                    ...
                    print ">>>>>>>>>>>>>>>>>2 Revert " ,revert_id 
                    revert_id= move_obj.search([('id','in',revert_id)])
                    print ">>>>>>>>>>>>>>>>>3 Revert " ,move_obj,revert_id,revert_id.id , move,move.id
